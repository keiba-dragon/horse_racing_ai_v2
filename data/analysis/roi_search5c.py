# -*- coding: utf-8 -*-
"""roi_search5 右回り/左回り版 - ポジティブグループ詳細"""
...
for tgt in ['ダ_中長_右','ダ_短_右','ダ_マイ_右']:
    g = oos_w[oos_w['gk']==tgt]
    r1e = g[g['rank_edge']==1]
    print(f'\n{SEP}')
    print(f' [{tgt}] (OOS N={len(g):,})')
    print(SEP)
    ...
    for yr in sorted(g['year'].unique()):
        sub = r1e[r1e['year']==yr]
        r,w,n = roi_tan(sub)
        cum_bet += n; cum_ret += w*(sub[sub['target_win']==1]['単勝オッズ'].sum()) if w>0 else 0
        # 正確な累計ROI計算
        r_cum,_,n_cum = roi_tan(r1e[r1e['year']<=yr])
        print(f'    20{yr}: N={n:>4}  勝={w:>3}  ROI={r:>+7.1%}  累計ROI={r_cum:>+7.1%}')
    ...
    print('  年別 × edge≥0.02:')
    for yr in sorted(g['year'].unique()):
        sub = r1e[(r1e['year']==yr)&(r1e['edge']>=0.02)]
        r,w,n = roi_tan(sub)
        print(f'    20{yr}: N={n:>4}  勝={w:>3}  ROI={r:>+7.1%}')
