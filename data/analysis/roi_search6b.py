...
for key, dist_focus in [('阪_ダ', 'マイ'), ('阪_芝', None)]:
    g_all  = oos[oos['gk'] == key]
    r1e_all = g_all[g_all['rank_edge'] == 1]

    if dist_focus:
        g   = g_all[g_all['距離帯'] == dist_focus]
        lbl = f'{key} [{dist_focus}距離のみ]'
    else:
        g   = g_all
        lbl = key
    ...
    print(f'\n{SEP}')
    print(f' {lbl}  OOS edge1位 N={len(r1e):,}')
    print(SEP)
    ...
    print()
    print('  オッズ帯別:')
    for lo, hi in [(2, 5), (5, 10), (10, 20), (20, 50), (3, 8), (5, 15), (3, 12)]:
    ...
    print()
    print('  クラス別:')
    if 'クラス_rank' in g.columns:
        cls_labels = {1:'未勝利', 2:'1勝', 3:'2勝', 4:'3勝/OP', 5:'重賞'}
        for cr in sorted(r1e['クラス_rank'].dropna().unique()):
            sub = r1e[r1e['クラス_rank'] == cr]
            r, w, n = roi_tan(sub)
            if n < 20: continue
            lbl2 = cls_labels.get(int(cr), f'rank{int(cr)}')
            print(f'    {lbl2}(rank={int(cr)}): N={n:>4,}  的中={sub["target_win"].mean():.1%}  ROI={r:>+7.1%}')
    ...
    print()
    print('  edge≥0.03 × オッズ帯 × 年別:')
    sub_e = r1e[r1e['edge'] >= 0.03]
    for lo, hi in [(3, 10), (5, 15), (10, 30)]:
        sub = sub_e[(sub_e['単勝オッズ'] >= lo) & (sub_e['単勝オッズ'] < hi)]
        r, w, n = roi_tan(sub)
        if n < 20: continue
        print(f'  [{lo}-{hi}倍 × edge≥0.03] N={n:,}  ROI={r:>+7.1%}')
        for yr in sorted(sub['year'].unique()):
            sy = sub[sub['year'] == yr]
            ry, wy, ny = roi_tan(sy)
            print(f'      20{yr}: N={ny:>3,}  的中={sy["target_win"].mean():.1%}  ROI={ry:>+7.1%}')

    # 距離帯別サマリー（阪_芝用）
    if dist_focus is None:
        print()
        print('  距離帯 × 年別 ROI (edge1位):')
        for db in ['短', 'マイ', '中', '長']:
            sub_d = r1e_all[r1e_all['距離帯'] == db]
            r, w, n = roi_tan(sub_d)
            if n < 30: continue
            print(f'  {SEP2}')
            print(f'  [{db}距離] N={n:,}  ROI={r:>+7.1%}')
            for yr in sorted(sub_d['year'].unique()):
                sy = sub_d[sub_d['year'] == yr]
                ry, wy, ny = roi_tan(sy)
                print(f'    20{yr}: N={ny:>3,}  的中={sy["target_win"].mean():.1%}  ROI={ry:>+7.1%}')
