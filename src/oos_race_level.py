...
print(f'\nOOSレース数: {n_races:,}  1着馬数: {len(winners):,}')
...
print()
print('【OOS レース単位: 1着馬の予測確率帯】')
print(f'{"確率帯":>8} {"レース数":>10} {"割合":>8}  | 馬単位勝率(参考)')
print('-' * 52)
for b in labels:
    gw = winners[winners['band'] == b]
    ga = valid[valid['band'] == b]
    nw = len(gw)
    na = len(ga)
    wa = int(ga['win'].sum())
    pct_race  = nw / len(winners) * 100
    pct_horse = wa / na * 100 if na > 0 else 0
    print(f'{b:>8} {nw:>10,} {pct_race:>7.1f}%  | {na:>8,}頭 {pct_horse:>6.2f}%')
