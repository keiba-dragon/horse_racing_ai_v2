# coding: utf-8
"""verify_da21f.py - 21F最終モデルの backward elimination 検証"""
import sys, os
import numpy as np
import pandas as pd
...
def main():
    ...
    nll0, beta0, roi0 = evaluate(df_trn, df_val, oos_parts, FINAL_21)
    r25_0 = roi0['2025'][0]; r26_0 = roi0['2026'][0]
    n25, n26 = roi0['2025'][1], roi0['2026'][1]
    comb0 = (r25_0*n25 + r26_0*n26)/(n25+n26)
    print(f'21F: valNLL={nll0:.5f}  2324:{roi0["2324"][0]:+.2%}  2025:{r25_0:+.2%}  2026:{r26_0:+.2%}  25+26:{comb0:+.2%}')

    valid0 = [c for c in FINAL_21 if c in df_trn.columns and df_trn[c].isna().mean() <= 0.65]
    print('\nベータ係数（絶対値降順）:')
    ...
    results = []
    for feat in FINAL_21:
        remaining = [f for f in FINAL_21 if f != feat]
        ...
        delta = comb - comb0
        both = (r25 > r25_0 and r26 > r26_0)
        results.append((feat, delta, comb, both, r25, r26))
    results.sort(key=lambda x: -x[1])
    print(f'{"削除特徴":<35} {"Δ25+26":>8}  合意? 結果')
    for feat, delta, comb, both, r25, r26 in results:
        sym = '✓削' if delta > 0.003 else ('✗' if delta < -0.003 else '~')
        agree = '[両✓]' if both else ''
        print(f'{sym} {feat:<33} {delta:>+8.2%}  {agree:<5} 25:{r25:+.2%} 26:{r26:+.2%} 25+26:{comb:+.2%}')

    improved = [f for f,d,_,_,_,_ in results if d > 0.003]
    print(f'\n削除改善({len(improved)}本): {improved}')

    # サマリ
    print(f'\n{"="*55}')
    print('=== 全体改善サマリ ===')
    print(f'v303:   2025:-28.10%  2026:-28.19%  25+26:-28.13%')
    print(f'21F:    2025:{r25_0:+.2%}  2026:{r26_0:+.2%}  25+26:{comb0:+.2%}')
    print(f'改善:   2025:{r25_0+0.2810:+.2%}  2026:{r26_0+0.2819:+.2%}  25+26:{comb0+0.2813:+.2%}')
