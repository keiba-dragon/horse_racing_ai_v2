# coding: utf-8
"""
save_da_21f.py - ダ_中長距離 21F最終モデルを保存
出力: models/exp_da21f/ダ_中長距離.pkl

特徴量選択経緯:
  v303(20F) → dead3削除 → 17F
  17F → backward: 上り3F削除 → 16F → backward: タイム指数平均削除 → 15F
  15F → forward: 近3走_複勝率 → 16F (exp_da16f)
  16F + 調教師_r200_複勝率 = 17F base
  17F → greedy forward (25+26 ROI): ブリンカー変更, 2走前_クラス差, 4走前_クラス差, 1走前_馬場状態 → 21F
  21F backward: 削除改善0本 (安定)

性能:
  2324: -23.31%  2025: -18.89%  2026: -21.81%  25+26: -19.70%
  vs v303: +8.43pp改善 (2025: +9.21pp, 2026: +6.38pp)
"""
...
OUT_DIR = os.path.join(BASE_DIR, 'models', 'exp_da21f')
...
def main():
    ...
    print(f'有効特徴: {len(valid)}/{len(FINAL_21)}')
    missing = [c for c in FINAL_21 if c not in valid]
    if missing:
        print(f'除外: {missing}')
    ...
    pkg = {
        ...
        'seg_key': 'ダ_中長距離',
        'val_nll': val_nll,
        'description': '21F最終モデル: v303から backward/forward で +8.43pp改善 (25+26=-19.70%)',
    }
