# coding: utf-8
"""
compare_shiba_mid3.py - 芝中距離（芝1401m-2000m）Round 3
* Round2 best: S=-2.88% (調教師コース+馬距離+クラス調整着順)
* S のバリエーションと追加候補で安定性確認
"""
import sys, os, time
import numpy as np
import pandas as pd
...
ALL_FEATS = [
    '1走前_3角', '1走前_脚質_num', '近5走_上り3F平均', '前走着差タイム',
    '1走前_クラス調整着順', '1走前_クラス差', '2走前_クラス差',
    'コース枠_r200_勝率', 'コース脚質_r200_勝率', '馬距離_勝率',
    '騎手コース_r100_勝率', '調教師コース_r100_勝率',
    '間隔', '距離変化_前走', '芝ダ転向',
    '近5走_クラス調整_平均着順', '近5走_クラス補正スコア',
]

# Round2: S=-2.88%(best), I=-19.99%, baseline=-22.21%
# S: 調教師コース_r100_勝率, 馬距離_勝率, 1走前_クラス調整着順
SETS = {
    'S: R2best(再確認)':          ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順'],
    'T: S+騎手コース':             ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順',
                                    '騎手コース_r100_勝率'],
    'U: S+脚質_num':              ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順',
                                    '1走前_脚質_num'],
    'V: S+前走着差':               ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順',
                                    '前走着差タイム'],
    'W: S+3角':                   ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順',
                                    '1走前_3角'],
    'X: S+クラス差':               ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順',
                                    '1走前_クラス差'],
    'Y: 調教師+馬距離のみ':         ['調教師コース_r100_勝率', '馬距離_勝率'],
    'Z: S+上り3F':                 ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順',
                                    '近5走_上り3F平均'],
    'AA: 調教師+騎手+馬距離':       ['調教師コース_r100_勝率', '騎手コース_r100_勝率', '馬距離_勝率'],
    'AB: S+5走クラス補正':          ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順',
                                    '近5走_クラス調整_平均着順'],
}
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("  芝中距離（芝1401m-2000m）Round 3")
    print("  Round2 best: S=-2.88% (調教師コース+馬距離+クラス調整着順)")
    print("  baseline=-22.21%  ※ 2324は参考のみ。25+26で優劣を判断する")
    print("=" * 72)
    ...
    print(f"\ntrain:{len(df_trn):,}行  val:{len(df_val):,}行")
    ...
    print(f"  baseline比: {(best_comb+0.2221)*100:+.2f}pp")
    print(f"  Round2 best比 (S=-2.88%): {(best_comb+0.0288)*100:+.2f}pp")
