# coding: utf-8
"""
try_age_market.py - 年齢・市場特徴テスト
  1. 年齢 (NaN=0%, 未テスト)
  2. log(単勝オッズ), 人気, 前走人気 を BASE_25 に追加
  3. 市場調整モデル: BASE_25 + log(単勝オッズ) の意味
"""
import sys, os
import numpy as np
import pandas as pd
...
def load_segment():
    ...
    # 追加計算特徴
    odds = pd.to_numeric(df.get('単勝オッズ', pd.Series(np.nan, index=df.index)), errors='coerce')
    df['log_単勝オッズ'] = np.log1p(odds)

    # 年齢の数値化
    df['年齢_num'] = pd.to_numeric(df.get('年齢', pd.Series(np.nan, index=df.index)), errors='coerce')

    # 人気の数値化
    df['人気_num'] = pd.to_numeric(df.get('人気', pd.Series(np.nan, index=df.index)), errors='coerce')
    df['前走人気_num'] = pd.to_numeric(df.get('前走人気', pd.Series(np.nan, index=df.index)), errors='coerce')

    # 前走人気変化 (今回-前回, 人気が上がった=マイナス)
    df['人気変化'] = df['人気_num'] - df['前走人気_num']
...
def main():
    ...
    # NaN確認
    for c in ['年齢_num', '人気_num', 'log_単勝オッズ', '前走人気_num', '人気変化']:
        print(f'{c}: NaN={df_trn[c].isna().mean():.1%}')
    print()

    print('=== 年齢・市場特徴テスト (BASE_25 + L2=0.006) ===')
    print()
