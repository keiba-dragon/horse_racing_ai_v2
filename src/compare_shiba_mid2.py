# coding: utf-8
"""
compare_shiba_mid2.py - 芝中距離（芝1401m-2000m）Round 2
* Round 1 最良: G(-19.92%), H(-21.14%) を参考に新セット設計
* 2324は参考のみ。25+26で優劣を判断する
"""
import sys, os, time
import numpy as np
import pandas as pd
...
ALL_FEATS = [
    ...
    '近5走_上り3F平均', '1走前_タイム指数', '前走着差タイム',
    '1走前_クラス調整着順', '1走前_クラス差', '2走前_クラス差',
    'コース枠_r200_勝率', 'コース脚質_r200_勝率', '馬距離_勝率', '馬距離_複勝率',
...
# Round1: G=-19.92%(best), H=-21.14%, baseline=-22.21%
# G: クラス差+脚質_num+馬距離_勝率 / H: 騎手+クラス調整+脚質
SETS = {
    'I: G+騎手':                 ['1走前_クラス差', '2走前_クラス差', '1走前_脚質_num',
                                   '馬距離_勝率', '騎手コース_r100_勝率'],
    'J: G-馬距離(2026安定化)':   ['1走前_クラス差', '2走前_クラス差', '1走前_脚質_num',
                                   '1走前_3角'],
    'K: C+脚質':                  ['1走前_クラス調整着順', '前走着差タイム', '近5走_上り3F平均',
                                    '1走前_脚質_num'],
    'L: H+距離変化':              ['騎手コース_r100_勝率', '1走前_クラス調整着順',
                                   '距離変化_前走', '1走前_脚質_num'],
    'M: クラス5走+脚質':          ['近5走_クラス調整_平均着順', '1走前_クラス差',
                                   '1走前_脚質_num', '馬距離_勝率'],
    'N: タイム+前走+脚質':        ['1走前_タイム指数', '前走着差タイム', '1走前_脚質_num'],
    'P: 馬距離+クラス+騎手':      ['馬距離_勝率', '1走前_クラス調整着順', '前走着差タイム',
                                   '騎手コース_r100_勝率'],
    'Q: G(best)再確認':           ['1走前_クラス差', '2走前_クラス差', '1走前_脚質_num',
                                   '馬距離_勝率'],
    'R: 斤量+クラス+脚質':        ['斤量', '1走前_クラス差', '1走前_脚質_num', '前走着差タイム'],
    'S: 調教師+馬距離+クラス':    ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_クラス調整着順'],
}
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("  芝中距離（芝1401m-2000m）Round 2")
    print("  Round1 best: G=-19.92%  baseline=-22.21%")
    ...
    print(f"  Round1 best比 (G=-19.92%): {(best_comb+0.1992)*100:+.2f}pp")
