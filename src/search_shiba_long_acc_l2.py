# coding: utf-8
"""
search_shiba_long_acc_l2.py - 芝長 L2=0.003 でランダムサーチ
usage: python src/search_shiba_long_acc_l2.py [N_ITER] [SEED]
"""
...
N_ITER  = int(sys.argv[1]) if len(sys.argv) > 1 else 800
SEED_RNG = int(sys.argv[2]) if len(sys.argv) > 2 else 42
...
def eval_feats(feats, dfs):
    ...
    def acc_oos(oos):
        ...
        t2 = s[s['rank'] == 1]; nr = s['race_id'].nunique()
        return (t2['着順_num'] == 1).mean(), nr
    a2324, n2324 = acc_oos(oos_2324); a25, n25 = acc_oos(oos_2025)
    return (a2324*n2324 + a25*n25) / (n2324+n25) if (n2324+n25) > 0 else float('-inf')


def main():
    print(f'ランダムサーチ L2={L2}: {SEG_NAME}  N_ITER={N_ITER}  SEED={SEED_RNG}  目標={FAV:.4f}')
    ...
    seed_feats = [f for f in SEED_FEATS if f in seg.columns]
    ...
    print(f'\n最終 L2={L2}: 芝長 acc_2325={best_score:.4f}  目標まで{FAV-best_score:+.4f} ({N_ITER}it, {time.time()-t0:.0f}s)')
    ...
    if best_score > existing_acc + 0.0001:
        ...
        # save with full train+val+oos_2026
        ...
        n2324 = results.get('2324', (0,0,0))[2]; n25 = results.get('2025', (0,0,0))[2]
        a2324 = results.get('2324', (0,0,0))[0]; a25 = results.get('2025', (0,0,0))[0]
        a26 = results.get('2026', (0,0,0))[0]; r25 = results.get('2025', (0,0,0))[1]
        r26 = results.get('2026', (0,0,0))[1]; n26 = results.get('2026', (0,0,0))[2]
        ...
        acc_pkg = {
            ...
            'oos_roi_2526': roi_2526, 'version': f'shiba_long_acc_L2_0003',
            'note': f'L2=0.003 rand: {len(best_feats)}特徴 acc_2325={acc_2325:.4f}',
        }
