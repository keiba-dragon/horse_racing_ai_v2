# coding: utf-8
"""
verify_da16f.py - 最終16特徴モデルの検証（backward elimination）
Feature: 15F + 近3走_複勝率
"""
import sys, os, time
import numpy as np
...
def main():
    ...
    # 16F評価
    nll0, beta0, roi0 = evaluate(df_trn, df_val, oos_parts, FINAL_16)
    base_comb = combined_roi(roi0)
    print(f'=== 最終16特徴モデル ===')
    print(f'valNLL={nll0:.5f}  {fmt(roi0)}')
    print()

    # ベータ係数
    valid0 = [c for c in FINAL_16 if c in df_trn.columns and df_trn[c].isna().mean() <= 0.65]
    print('ベータ係数（絶対値降順）:')
    ...
    print('=== backward elimination 検証 ===')
    print(f'{"削除特徴":<35} {"valNLL":>9} {"Δ25+26":>8}  {"結果"}')
    print('-'*90)
    results = []
    for feat in FINAL_16:
        remaining = [f for f in FINAL_16 if f != feat]
    ...
    print()
    improving = [f for f, _, d, _ in results if d > 0.003]
    if improving:
        print(f'削除改善: {improving}')
    else:
        print('削除で改善する特徴なし → 16特徴が安定最適解')

    # 比較サマリ
    print()
    print('=== 改善サマリ ===')
    print(f'v303ベースライン:       2025:-28.10%  2026:-28.19%  25+26:-28.13%')
    print(f'17F(dead3本除去後):     2025:-27.51%  2026:-26.66%  25+26:-27.27%')
    print(f'16F最終:               2025:{roi0.get("2025",(0,0))[0]:+.2%}  '
          f'2026:{roi0.get("2026",(0,0))[0]:+.2%}  25+26:{base_comb:+.2%}')
    print(f'改善幅(vs v303):  2025:{roi0.get("2025",(0,0))[0]+0.2810:+.2%}  '
          f'2026:{roi0.get("2026",(0,0))[0]+0.2819:+.2%}  '
          f'25+26:{base_comb+0.2813:+.2%}')
