# coding: utf-8
"""
valloss_da_short.py - ダート短距離 バリデーション損失版 Forward Greedy
* 選択基準: 2022年バリデーション log-loss の最小化のみ
* 2323/24/25/26 は一切触れない（全部ブラインド）
* 2324の天井問題を回避し、より多くの特徴量を採用できる
...
L2 = 0.006
VAL_LOSS_THRESHOLD = 0.0005  # Δval_loss < -0.0005 で採用
...
def adam_fit(X_tr, y_tr, gs_tr, n_tr, nr_tr, X_va, y_va, gs_va, n_va, nr_va, l2=L2):
    """Returns (best_beta, best_val_loss)"""
    d = X_tr.shape[1]
...
def evaluate_valloss(df_trn, df_val, feats):
    """2022バリデーション損失で評価（選択基準用）"""
    ...
    beta, val_loss = adam_fit(X_tr, y_tr, gs_tr, n_tr, nr_tr,
                               X_va, y_va, gs_va, n_va, nr_va)
    return val_loss, valid, beta


def evaluate_blind_all(df_trn, df_val, oos_2324, oos_2025, oos_2026, feats):
    """全OOS期間のブラインド評価（最終のみ）"""
    valid = [c for c in feats if c in df_trn.columns and df_trn[c].isna().mean() <= 0.65]
    if not valid:
    ...
    beta, _ = adam_fit(X_tr, y_tr, gs_tr, n_tr, nr_tr,
                        X_va, y_va, gs_va, n_va, nr_va)
    results = []
    for oos in [oos_2324, oos_2025, oos_2026]:
        if len(oos) == 0:
    ...
    (r2324, n2324), (r25, n25), (r26, n26) = results
    return r2324, r25, r26, n2324, n25, n26


def main():
    t0 = time.time()
    print("=" * 65)
    print("  ダート短距離 バリデーション損失版 Forward Greedy")
    print(f"  選択基準: Δval_loss < -{VAL_LOSS_THRESHOLD:.4f} のみ（2022年）")
    print("  2323/24/25/26 は全部ブラインド")
    print("=" * 65)
    print()
    ...
    print(f"[ブラインド] 2324:{len(oos_2324):,}  2025:{len(oos_2025):,}  2026:{len(oos_2026):,}")
    print()
    ...
    # ─── Round 1: 最良シングルトン（val_loss最小） ────────────────────
    print("─" * 65)
    print("  Round 1: シングルトン評価 (val_loss最小を採用)")
    print("─" * 65)

    best_feat = None
    best_valloss = np.inf
    for cand in avail:
        vl, valid_t, _ = evaluate_valloss(df_trn, df_val, [cand])
        if vl is None or cand not in valid_t or np.isnan(vl):
            continue
        print(f"  [{cand}] val_loss:{vl:.6f}")
        if vl < best_valloss:
            best_valloss = vl
            best_feat = cand
    ...
    print(f"\n  → 採用: {best_feat}  val_loss:{best_valloss:.6f}\n")
    current = [best_feat]
    remaining = [c for c in avail if c not in current]

    # ─── Round 2+: Forward Greedy (val_loss最小化) ───────────────────
    print("─" * 65)
    print(f"  Forward Greedy (採用基準: Δval_loss < -{VAL_LOSS_THRESHOLD:.4f})")
    print("─" * 65)

    round_num = 2
    while remaining:
        print(f"\n--- Round {round_num}: {len(current)}特徴, val_loss:{best_valloss:.6f} ---")
        best_delta = np.inf
        best_cand_this = None
        best_valid_this = None

        for cand in remaining:
            vl, valid_t, _ = evaluate_valloss(df_trn, df_val, current + [cand])
            if vl is None or np.isnan(vl):
                continue
            actually_added = cand in valid_t
            delta = vl - best_valloss  # 負が良い
            good = delta < -VAL_LOSS_THRESHOLD and actually_added
            sym = '✓' if good else '✗'
            note = '' if actually_added else ' [NaN>65%]'
            print(f"  {sym} +{cand}{note}  val_loss:{vl:.6f}(Δ{delta:+.6f})")
        ...
        best_valloss = best_valloss_new
        remaining = [c for c in remaining if c not in current]
        print(f"\n  → 採用: {best_cand_this}  ({len(current)}特徴)  val_loss:{best_valloss:.6f}")
        round_num += 1
    ...
    print(f"\n  val_loss (選択基準 2022): {best_valloss:.6f}")

    # ─── ブラインド評価 (2324/2025/2026) ────────────────────────────
    print(f"\n{'─' * 65}")
    print("  ブラインド評価 (2324/2025/2026) ← 選択中は一度も見ていない")
    print(f"{'─' * 65}")
    r2324, r25, r26, n2324, n25, n26 = evaluate_blind_all(
    ...
    print(f"  safe_da_short.py比: {rcomb*100:.2f}% vs -6.36%  (Δ{(rcomb+0.0636)*100:+.2f}pp)")
