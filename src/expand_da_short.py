# coding: utf-8
"""
expand_da_short.py - ダート短距離 4特徴起点 条件緩和Forward Greedy
START: [近5走_上り3F平均, 3走前_クラス差, キャリア_浅い, 2走前_クラス差]
採用基準: Δ25+26 > 0 かつ Δ2324 >= -5pp (25%以上を維持)
"""
import sys, os, time
import numpy as np
import pandas as pd
...
ADD_CANDIDATES = [
    ...
    'ブリンカー変更', '1走前_クラス差', '4走前_クラス差', '1走前_馬場状態',
    '性別_num', '所属_num', 'タイム指数_近5走_slope',
    ...
    '距離変化_前走', '芝ダ転向', '3走前_クラス差', 'クラス_rank', '格上経験数_近5走',
...
def main():
    t0 = time.time()
    print("=== ダート短距離 条件緩和 Forward Greedy (4特徴起点) ===\n")
    print("採用基準: Δ25+26 > 0 かつ Δ2324 >= -5pp (25%以上維持)\n")
    df = load_segment()
    ...
    avail_add = [c for c in ADD_CANDIDATES if c in df_trn.columns and
                 df_trn[c].notna().sum() > 1000 and c not in START_FEATS]

    # 初期評価
    res0 = evaluate(df_trn, df_val, oos_2324, oos_2025, oos_2026, START_FEATS)
    _, _, r2324_0, r25_0, r26_0, ns_0 = res0
    n25_0, n26_0 = ns_0[1], ns_0[2]
    comb_0 = comb2526(r25_0, n25_0, r26_0, n26_0)
    print(f"\n開始(4特徴): 2324:{r2324_0*100:.2f}%  2025:{r25_0*100:.2f}%  2026:{r26_0*100:.2f}%  25+26:{comb_0*100:.2f}%\n")

    current = list(START_FEATS)
    best_2324 = r2324_0
    best_comb = comb_0
    remaining = avail_add.copy()

    round_num = 1
    total_added = 0
    while remaining:
        print(f"\n--- Round {round_num}: 現在 {len(current)}特徴, 2324:{best_2324*100:.2f}%, 25+26:{best_comb*100:.2f}% ---")
        best_delta_comb = -999.0
        best_cand_this = None
        best_state_this = None

        for cand in remaining:
            res = evaluate(df_trn, df_val, oos_2324, oos_2025, oos_2026, current + [cand])
            if res[0] is None:
                continue
            _, valid_t, r2324, r25, r26, ns = res
            actually_added = cand in valid_t
            n25, n26 = ns[1], ns[2]
            rcomb = comb2526(r25, n25, r26, n26)
            delta_2324 = r2324 - best_2324
            delta_comb = rcomb - best_comb
            # 採用基準: Δ25+26 > 0 かつ Δ2324 >= -5pp
            good = delta_comb > 0 and delta_2324 >= -0.05 and actually_added
            sym = '✓' if good else '✗'
            note = '' if actually_added else ' [NaN>65%]'
            print(f"  {sym} +{cand}{note}  2324:{r2324*100:.2f}%(Δ{delta_2324*100:+.2f}%)  "
                  f"2025:{r25*100:.2f}%  2026:{r26*100:.2f}%  "
                  f"25+26:{rcomb*100:.2f}%(Δ{delta_comb*100:+.2f}%)")
            if good and delta_comb > best_delta_comb:
                best_delta_comb = delta_comb
                best_cand_this = cand
                best_state_this = (r2324, rcomb, list(valid_t))

        if best_cand_this is None:
            print("  → 採用なし。Forward完了。")
            break

        best_2324, best_comb, current = best_state_this
        remaining = [c for c in remaining if c not in current]
        total_added += 1
        print(f"\n  → 採用: {best_cand_this}  ({len(current)}特徴)  "
              f"2324:{best_2324*100:.2f}%  25+26:{best_comb*100:.2f}%")
        round_num += 1

    # 最終評価
    res_final = evaluate(df_trn, df_val, oos_2324, oos_2025, oos_2026, current)
    print(f"\n{'='*65}")
    print(f"総時間: {int(time.time()-t0)}s")
    print(f"最終特徴量 ({len(current)}個):")
    for f in current:
        print(f"  {f}")
    if res_final[0] is not None:
        _, _, r2324, r25, r26, ns = res_final
        n25, n26 = ns[1], ns[2]
        rcomb = comb2526(r25, n25, r26, n26)
        print(f"\n最終結果:")
        print(f"  2324: {r2324*100:.2f}%  ({ns[0]}R)")
