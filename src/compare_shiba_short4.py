# coding: utf-8
"""
compare_shiba_short4.py - 芝短距離 Round4
* Round3 best: I=nv1+間隔+近5走上3F (6特徴, 25+26=+14.85%, タイ=1.6%)
* ベース I に 1〜2 特徴ずつ追加して 8〜10 特徴を目指す
* 騎手/調教師/馬距離WR/着差タイム は前ラウンドで有害確認済み → 除外
"""
import sys, os, time
import numpy as np
import pandas as pd
...
ALL_FEATS = [
    ...
    '2走前_クラス差', '1走前_クラス差',
    'コース枠_r200_勝率', 'コース脚質_r200_勝率',
    '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
    '1走前_脚質_num', '1走前_3角',
]

# Round3 best: I = nv1+間隔+近5走上3F (6特徴, +14.85%, タイ=1.6%)
BASE_I = ['1走前_3角', '芝ダ転向', '距離変化_前走', '1走前_脚質_num', '間隔', '近5走_上り3F平均']

SETS = {
    'I: R3best(6特徴,参考)':        BASE_I,
    'P1: I+馬体重':                  BASE_I + ['馬体重'],
    'P2: I+斤量':                    BASE_I + ['斤量'],
    'P3: I+馬番':                    BASE_I + ['馬番'],
    'P4: I+同会場着順':               BASE_I + ['同会場_平均着順_近5走'],
    'P5: I+良馬場着順':               BASE_I + ['良馬場_平均着順_近5走'],
    'P6: I+2走クラス差':              BASE_I + ['2走前_クラス差'],
    'P7: I+コース脚質WR':             BASE_I + ['コース脚質_r200_勝率'],
    'P8: I+馬体重+斤量':              BASE_I + ['馬体重', '斤量'],
    'P9: I+馬体重+馬番':              BASE_I + ['馬体重', '馬番'],
    'P10: I+同会場+良馬場':           BASE_I + ['同会場_平均着順_近5走', '良馬場_平均着順_近5走'],
    'P11: I+2走クラス差+馬体重':       BASE_I + ['2走前_クラス差', '馬体重'],
    'P12: I+同会場+馬体重+斤量':      BASE_I + ['同会場_平均着順_近5走', '馬体重', '斤量'],
    'P13: I+馬体重+斤量+馬番':        BASE_I + ['馬体重', '斤量', '馬番'],
    'P14: I+同会場+良馬場+馬体重+斤量': BASE_I + ['同会場_平均着順_近5走',
                                                  '良馬場_平均着順_近5走', '馬体重', '斤量'],
}
...
def main():
    ...
    NV1_ROI = +0.1485
    print("=" * 82)
    print("  芝短距離 Round4 — ベースI(6特徴,+14.85%)に追加特徴を探索")
    print(f"  ベースI: {BASE_I}")
    ...
    print(f"\n{'='*82}")
    print(f"  {'セット':34s}  {'2324':>7}  {'2025':>7}  {'2026':>7}  {'25+26':>7}  "
          f"{'タイ率':>6}  特徴数")
    print(f"  {'-'*78}")
    ...
    for name, feats in SETS.items():
        ...
        print(f"  {name:34s}  {r2324*100:+6.2f}%  {r25*100:+6.2f}%  {r26*100:+6.2f}%  "
    ...
    print(f"\n{'='*82}")
    print(f"  Round4 ベスト: {best_name}  25+26={best_comb*100:.2f}%")
