# coding: utf-8
"""
try_jockey_upgrade.py - 騎手変更の方向性・馬主勝率テスト
  - 騎手アップグレード度 = 現騎手_r200_勝率 - 前騎手_r200_勝率
    (前走から騎手が良くなったか悪くなったか)
  - 馬主_勝率 (訓練データから計算)
  - 騎手コース_r100_複勝率 (未テスト)
  - 騎手コース_r100_勝率 - 調教師コース_r100_勝率 の差分
"""
import sys, os
import numpy as np
import pandas as pd
...
def load_segment():
    ...
    # 騎手アップグレード度: 現騎手_r200_勝率 - 前騎手_r200_勝率
    # 前騎手の勝率は 前騎手名 → 騎手_r200_勝率 でマッピング
    # まず騎手名→勝率のマップを作る (全体から)
    jockey_wr = df.dropna(subset=['騎手', '騎手_r200_勝率']).groupby('騎手')['騎手_r200_勝率'].mean()
    jockey_wr_map = jockey_wr.to_dict()

    curr_jwr = pd.to_numeric(df.get('騎手_r200_勝率', pd.Series(np.nan, index=df.index)), errors='coerce')
    prev_jockey = df.get('前騎手', pd.Series('', index=df.index)).astype(str)
    prev_jwr = prev_jockey.map(jockey_wr_map)

    df['騎手アップグレード'] = curr_jwr - prev_jwr

    # 騎手コース_r100_複勝率 (複勝率版, 未テスト)
    # 既にカラムあれば使う

    # 騎手_r200_勝率 (全体勝率) vs 調教師コース勝率 の差
    df['騎手調教師_勝率差'] = (pd.to_numeric(df.get('騎手_r200_勝率', pd.Series(np.nan, index=df.index)), errors='coerce') -
                              pd.to_numeric(df.get('調教師コース_r100_勝率', pd.Series(np.nan, index=df.index)), errors='coerce'))

    return df


def compute_owner_winrate(df_trn, df_all):
    """訓練データから馬主別勝率を計算"""
    owner_col = '馬主(最新/仮想)'
    if owner_col not in df_trn.columns:
        return df_all
    owner_stats = df_trn.groupby(owner_col).apply(
        lambda x: (x['着順_num'] == 1).sum() / len(x)
    ).reset_index()
    owner_stats.columns = [owner_col, '馬主_勝率']
    global_avg = owner_stats['馬主_勝率'].mean()
    owner_map = owner_stats.set_index(owner_col)['馬主_勝率'].to_dict()
    df_all = df_all.copy()
    df_all['馬主_勝率'] = df_all[owner_col].map(owner_map).fillna(global_avg)
    return df_all
...
def main():
    ...
    # 馬主勝率を訓練データから計算
    df = compute_owner_winrate(df_trn, df)
    df_trn = df[(df['日付_num'] >= 130101) & (df['日付_num'] < 220101)]
    ...
    print('=== 騎手変更方向性・馬主勝率テスト (BASE_25 + L2=0.006) ===')
    print()
