...
FEATS = ['近5走_上り3F平均', 'コース枠_r200_勝率', '1走前_馬場状態', '1走前_クラス差', '2走前_クラス差']
L2 = 0.006
...
def main():
    print('=' * 60)
    print('  ダート短距離 nv2 モデル保存')
    print(f'  特徴量: {FEATS}')
    print(f'  セグメント: ダ ≤1400m (新馬除外)')
    print('=' * 60)
    ...
    da_short_art = {
        ...
        'isotonic':      iso,
        'segment':       'ダ短 ≤1400m (新馬除外)',
        'oos_roi_2526':  comb,
    }

    new_artifacts = dict(existing_pkg['artifacts'])
    new_artifacts['ダ短'] = da_short_art
    ...
    new_pkg = {
        ...
        'note':          existing_pkg.get('note', '') + ' | ダ短nv2追加',
        'version':       'da_short_nv2',
    }
    ...
    print()
    print('=== ダート短距離 nv2 保存完了 ===')
    print(f'  ダ短 artifact: nv2 (5特徴, L2=0.006, isotonic calibrated)')
    print(f'  25+26 OOS ROI: {comb:+.2%}  (ベースライン比 +11.51pp vs -14.56%)')
