...
EXCLUDE = {
    ...
    '上3F地点差', '上3F地点差_x', '上3F地点差_y',  # 当レース中3F通過時点の差 → レース結果 → リーク
    'タイム指数', '上り3F_指数',
    ...
    # ── 他特徴量の変換/定数との演算 → clogit内で冗長 ──
    '所属_num',           # JRAレースほぼ全馬=1、実質定数
    '推定_脚質率',         # 近5走_平均4角位置 ÷ 頭数（線形スケーリングのみ）
    'レース内_相対脚質',   # レース内_平均脚質(定数) - 近5走_平均4角位置（符号反転）
    '展開フィット_v2',     # 推定ペース(定数)と推定_脚質率の差（近5走_平均4角位置の変換）
    '展開_コース_脚質フィット',  # 展開フィット_v2 × コース展開マッチ(定数)（定数倍のみ）
    '相手レベル_平均着順',  # (レース合計-自馬値)/(n-1) → 近5走_クラス調整_平均着順の符号反転と等価
    # ── 騎手距離_r100: 騎手_r200 と完全重複（r=1.000）→ 削除 ──
    '騎手距離_r100_勝率', '騎手距離_r100_複勝率',
    # ── クラスランク・クラス差: ノイズ（市場に既に織り込み済み） ──
    '1走前_クラス_rank', '2走前_クラス_rank', '3走前_クラス_rank', '4走前_クラス_rank',
    '5走前_クラス_rank', '6走前_クラス_rank', '7走前_クラス_rank', '8走前_クラス_rank',
    '9走前_クラス_rank', '10走前_クラス_rank',
    '1走前_クラス差', '2走前_クラス差', '3走前_クラス差', '4走前_クラス差', '5走前_クラス差',
    '最大クラス差_近5走',
}
