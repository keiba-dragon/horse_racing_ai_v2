# coding: utf-8
"""
search_3seg_acc_v2.py - 芝短・ダ長・ダ短 的中率探索 新候補追加版
目標: 各セグメントで1番人気的中率を超える
新候補: 1走前_単勝オッズ, PCI系, 個別過去走タイム指数, 前走上り3F等
"""
...
FAV = {'芝短': 0.2869, 'ダ長': 0.3403, 'ダ短': 0.3490}
SEG_VERSIONS = {'芝短': 'shiba_short_acc_v2', 'ダ長': 'da_long_acc_v8', 'ダ短': 'da_short_acc_v4'}
...
def save_seg(name, feats, seg):
    ...
    print(f'  保存完了: {name} v={SEG_VERSIONS[name]}')
    return acc_2325


def main():
    print('芝短・ダ長・ダ短 的中率探索 v2 (新候補追加)')
    ...
    for name in ['芝短', 'ダ長', 'ダ短']:
        ...
        best_score, _, _, _ = eval_feats(current, dfs)
        ...
        while len(current) < MAX_FEATS and remaining:
            ...
            for cand in remaining:
                score, beta, sc, valid = eval_feats(current + [cand], dfs)
            ...
            print(f'  +{best_add:35s}  {best_score:.4f}  gap={gap:+.4f}  ({len(current)}f, {time.time()-t0:.0f}s)')
            sys.stdout.flush()

            if best_score >= FAV[name]:
                print(f'  *** 1番人気超え達成! {best_score:.4f} > {FAV[name]:.4f} ***')
                break

        summary[name] = (best_score, len(best_feats))
        print(f'\n  最終: {name} acc_2325={best_score:.4f}  目標まで{FAV[name]-best_score:+.4f}')
