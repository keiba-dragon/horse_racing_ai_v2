# coding: utf-8
"""
fresh_shiba_1200.py - 芝1200m Forward Greedy
* 選択基準: Δ2324 > +0.5pp
* 2025/26 は最後にブラインド評価
* ベースライン: 芝短全体 -26.54%
"""
...
def load_segment():
    ...
    # 芝1200mのみ
    df = df[(df['surface'] == '芝') & (dm == 1200)].copy()
...
def main():
    ...
    print(f"train:{len(df_trn):,}行({df_trn['race_id'].nunique()}R)  "
          f"val:{len(df_val):,}行({df_val['race_id'].nunique()}R)  "
          f"2324:{len(oos_2324):,}行({oos_2324['race_id'].nunique()}R)")
    print(f"[ブラインド] 2025:{len(oos_2025):,}行({oos_2025['race_id'].nunique()}R)  "
          f"2026:{len(oos_2026):,}行({oos_2026['race_id'].nunique()}R)")
    print()

    avail_raw = [c for c in ALL_CANDIDATES if c in df_trn.columns and
                 df_trn[c].notna().sum() > 500]
