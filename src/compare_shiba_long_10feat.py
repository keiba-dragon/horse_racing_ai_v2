# coding: utf-8
"""
compare_shiba_long_10feat.py - 芝長距離 10特徴探索
  選択指標: 2323 OOS ROI  (25+26は報告のみ)
  現Y8(5特徴)をベースに greedy + pair search で 10 特徴を目指す
  セグメント: 芝 > 2000m
"""
...
BASE_Y8 = ['前走着差タイム', '距離変化_前走', '1走前_クラス差', '馬距離_勝率', '間隔']

CANDIDATES = [
    '性別_num', '斤量', '馬番', '馬体重',
    '2走前_クラス差', '3走前_クラス差',
    '1走前_脚質_num', '1走前_3角',
    '近5走_上り3F平均', '近5走_クラス調整_平均着順', '1走前_クラス調整着順',
    'コース脚質_r200_勝率', 'コース枠_r200_勝率',
    '同会場_平均着順_近5走', '良馬場_平均着順_近5走',
    '調教師コース_r100_勝率',
    '近5走_上り3F_std', '1走前_タイム指数',
    '芝ダ転向',
]
...
def main():
    t0 = time.time()
    print("=" * 90)
    print("  芝長距離 10特徴探索 — 選択指標=2323 OOS ROI")
    print(f"  ベースY8(5特徴): {BASE_Y8}")
    print("=" * 90)
    ...
    r_base, c_base, r25_b, r26_b, valid_base, _ = eval_feats(
        df_trn, df_val, oos_2324, oos_2025, oos_2026, BASE_Y8)
    print(f"\n[ベースY8(5特徴)] 2323={r_base*100:+.2f}%  25+26={c_base*100:+.2f}%")

    selected = list(BASE_Y8)
    remaining = list(CANDIDATES)

    print("\n=== Greedy forward selection (2323選択) ===")
    while len(selected) < 10 and remaining:
        ...
        print(f"\n[{len(selected)}特徴 → 10目標] 全候補:")
        print(f"  {'追加候補':30s}  {'2323':>8}  {'25+26':>8}")
        for feat, r2324, c2526, r25, r26, valid, beta in round_res:
            marker = " ←BEST" if feat == round_res[0][0] else ""
            print(f"  +{feat:29s}  {r2324*100:+7.2f}%  {c2526*100:+7.2f}%{marker}")
        best = round_res[0]
        if not np.isnan(best[1]) and best[1] > r_base:
            ...
            print(f"\n  ✗ greedy改善なし。残り{10-len(selected)}枠をペア検索へ")
            break

    # Pair/trio search for remaining slots
    slots = 10 - len(selected)
    if slots > 0 and len(remaining) >= slots:
        print(f"\n=== Pair search: 残り{len(remaining)}候補から{slots}個選択 ===")
        if slots == 1:
            combos = [(r,) for r in remaining]
        elif slots == 2:
            combos = list(itertools.combinations(remaining, 2))
        else:
            combos = list(itertools.combinations(remaining, min(slots, 3)))

        pair_res = []
        print(f"  {'組み合わせ':50s}  {'2323':>8}  {'25+26':>8}")
        for combo in combos:
            trial = selected + list(combo)
            r2324, c2526, r25, r26, valid, beta = eval_feats(
                df_trn, df_val, oos_2324, oos_2025, oos_2026, trial)
            print(f"  {'+'.join(combo)[:49]:50s}  {r2324*100:+7.2f}%  {c2526*100:+7.2f}%")
        ...
        print(f"\n  ✓ 採用: {best_combo[2]}  2323={best_combo[0]*100:+.2f}%")
