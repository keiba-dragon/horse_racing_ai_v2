# coding: utf-8
"""
search_da_long_shiba_long_acc_v2.py - ダ長・芝長 正しいSEEDで新候補追加探索

ダ長: da_long_acc_v7 (36特徴, acc_2325=30.02%) から新候補追加
芝長: shiba_long_acc_v1 (30特徴, acc_2325=34.75%) から新候補追加
目標: ダ長34.03%, 芝長36.05%
"""
...
# 正しいSEED（hitrate_model.pkl から取得した feat_cols ベース）
SEEDS = {
    'ダ長': [
        '馬番', '斤量', '近3走_複勝率', '騎手コース_r100_勝率', '1走前_クラス調整着順',
        ...
        '馬_r20_勝率', '性別_num', '騎手会場_r100_勝率', '相手レベル_平均着順',
    ],
    '芝長': [
...
FAV = {'ダ長': 0.3403, '芝長': 0.3605}
SEG_VERSIONS = {'ダ長': 'da_long_acc_v8', '芝長': 'shiba_long_acc_v2'}
...
def save_seg(name, feats, seg):
    ...
    acc_pkg = {
        ...
        'note': f'{SEG_VERSIONS[name]}: {len(feats)}特徴 acc_2325={acc_2325:.4f} 1番人気{FAV[name]:.2%}',
    }
    ...
    print(f'  保存: {name} {SEG_VERSIONS[name]}')
    return acc_2325


def main():
    print('ダ長・芝長 正しいSEEDで新候補追加探索 v2')
    print(f'MAX_FEATS={MAX_FEATS}  目標: ダ長34.03%, 芝長36.05%')
    ...
    seg_filters = {
        'ダ長': (df['surface'] == 'ダ') & (dm > 1400) & (df['クラス_rank'] != 1.0),
    ...
    for name in ['ダ長', '芝長']:
        print(f'\n{"="*60}')
        print(f'  {name}  目標: {FAV[name]:.4f} ({FAV[name]:.2%})')
        print(f'{"="*60}')

        seg = df[seg_filters[name]].copy()
        ...
        while len(current) < MAX_FEATS and remaining:
            ...
            for cand in remaining:
                score, beta, sc = eval_feats(current + [cand], dfs)
            ...
            if best_score >= FAV[name]:
                print(f'  *** 1番人気超え達成! {name}: {best_score:.4f} > {FAV[name]:.4f} ***')
                break

        summary[name] = (best_score, len(best_feats))
        ...
        if best_score > existing_acc + 0.0001:
            ...
            save_seg(name, best_feats, seg)
        else:
            print(f'  改善なし ({existing_acc:.4f} >= {best_score:.4f})、スキップ')

    print('\n' + '=' * 60)
    print('ダ長・芝長 完了')
    for n, (s, nf) in summary.items():
        gap = FAV[n] - s
        mark = '★超え' if s >= FAV[n] else f'gap={gap:+.4f}'
        print(f'  {n}: acc_2325={s:.4f} ({nf}特徴) {mark}')
