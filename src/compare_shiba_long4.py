# coding: utf-8
"""
compare_shiba_long4.py - 芝長距離（芝2001m以上）Round 4
* Round3 best: X=-14.04% (前走着差タイム, 距離変化_前走, 1走前_クラス差)
* X のバリエーションで安定性確認・さらなる改善探索
...
# X: 前走着差タイム, 距離変化_前走, 1走前_クラス差 → -14.04% (filtered比+6.50pp)
SETS = {
    'X: R3best(再確認)':           ['前走着差タイム', '距離変化_前走', '1走前_クラス差'],
    'X2: X+3角':                   ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                    '1走前_3角'],
    'X3: X+騎手':                  ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                    '騎手コース_r100_勝率'],
    'X4: X+脚質':                  ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                    '1走前_脚質_num'],
    'X5: X+馬距離':                ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                    '馬距離_勝率'],
    'X6: X-距離変化(2特徴)':        ['前走着差タイム', '1走前_クラス差'],
    'X7: X-クラス差(2特徴)':        ['前走着差タイム', '距離変化_前走'],
    'X8: X+クラス調整着順':          ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                    '1走前_クラス調整着順'],
    'X9: 2走前クラス差に替換':        ['前走着差タイム', '距離変化_前走', '2走前_クラス差'],
    'X10: X+2走前クラス差':          ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '2走前_クラス差'],
}
...
def main():
    ...
    FILTERED_BL = -0.2053
    print("=" * 72)
    print("  芝長距離（芝2001m以上）Round 4")
    print(f"  Round3 best X=-14.04%  filtered baseline={FILTERED_BL*100:.2f}%")
    ...
    print(f"  Round3 best比 (X=-14.04%): {(best_comb+0.1404)*100:+.2f}pp")
