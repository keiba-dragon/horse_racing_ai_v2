# coding: utf-8
"""
search_shiba_mid_2326.py - 芝中距離 特徴量探索 (2326選択指標)

v3.0: 2026も選択指標に組み込み。単年±30%キャップ付き。
  セグメント: 芝 1401m-2000m & クラス_rank!=1.0（新馬除外）
"""
import sys, os, time
import numpy as np
import pandas as pd
...
CANDIDATES = [
    ...
    '馬体重', '芝ダ転向', '距離変化_前走',
    # その他
    '性別_num', '騎手変更', 'ブリンカー変更', '種牡馬_勝率', '近3走_複勝率',
]
...
def load_segment():
    ...
    df = df[(df['surface'] == '芝') & (dm > 1400) & (dm <= 2000) & (df['クラス_rank'] != 1.0)].copy()
...
def eval_feats(df_trn, df_val, oos_2324, oos_2025, oos_2026, feats):
    ...
    # 選択指標: 2023-26合算。単年ROIをキャップして外れ値年の支配を防ぐ
    ...
    r26c   = np.clip(r26,   -ROI_CAP, ROI_CAP) if not np.isnan(r26)   else float('nan')
    r2326  = ((r2324c * n2324 + r25c * n25 + r26c * n26) /
              (n2324 + n25 + n26)) if (n2324 + n25 + n26) > 0 else float('nan')
    c2526  = (r25 * n25 + r26 * n26) / (n25 + n26) if (n25 + n26) > 0 else float('nan')
    return r2326, r2324, c2526, r25, r26, valid, beta


def main():
    t0 = time.time()
    print("=" * 90)
    print("  芝中距離 特徴量探索 v3.0 — 選択指標=2326(2023-26合算) OOS ROI")
    print("  2026も選択指標に組み込み。単年±30%キャップ付き。")
    print("=" * 90)
    ...
    best_r2326 = float('-inf')

    r_base, r2324_b, c_b, r25_b, r26_b, valid_b, _ = eval_feats(
        df_trn, df_val, oos_2324, oos_2025, oos_2026, selected)
    print(f"\n[強制ベース {FORCED_BASE}]  2326={r_base*100:+.2f}%  2323={r2324_b*100:+.2f}%  "
          f"2025={r25_b*100:+.2f}%  2026={r26_b*100:+.2f}%")
    best_r2326 = r_base if not np.isnan(r_base) else float('-inf')

    print("\n=== Greedy forward selection (2326キャップ選択) ===")
    while len(selected) < MAX_FEATS and remaining:
        round_res = []
        for feat in remaining:
            r2326, r2324, c2526, r25, r26, valid, beta = eval_feats(
                df_trn, df_val, oos_2324, oos_2025, oos_2026, selected + [feat])
            round_res.append((feat, r2326, r2324, c2526, r25, r26, valid, beta))
        round_res.sort(key=lambda x: -x[1] if not np.isnan(x[1]) else 999)

        print(f"\n[{len(selected)}特徴 → {MAX_FEATS}目標] 上位10候補:")
        print(f"  {'追加候補':30s}  {'2326(選択)':>10}  {'2323':>8}  {'2025':>8}  {'2026':>8}")
        for feat, r2326, r2324, c2526, r25, r26, *_ in round_res[:10]:
            marker = " ←BEST" if feat == round_res[0][0] else ""
            print(f"  +{feat:29s}  {r2326*100:+9.2f}%  {r2324*100:+7.2f}%  "
                  f"{r25*100:+7.2f}%  {r26*100:+7.2f}%{marker}")

        best = round_res[0]
        if not np.isnan(best[1]):
            improved = best[1] > best_r2326
            mark = "✓" if improved else "△"
            selected.append(best[0])
            remaining.remove(best[0])
            if improved:
                best_r2326 = best[1]
            print(f"\n  {mark} 採用: +{best[0]}  2326={best[1]*100:+.2f}%  "
    ...
    r2326_f, r2324_f, c2526_f, r25_f, r26_f, valid_f, beta_f = eval_feats(
        df_trn, df_val, oos_2324, oos_2025, oos_2026, selected)
    print(f"\n  2326(選択)={r2326_f*100:+.2f}%  2323={r2324_f*100:+.2f}%  "
