# coding: utf-8
"""
search_da_long_2325_v2.py - ダ長距離 特徴量探索 (2325選択指標 v2)

v2: 1走前_クラス差を強制ベースに追加（ユーザー指摘: 直近の格変化が最重要）
セグメント: ダ & 距離>1400m & クラス_rank≠1.0
選択指標: r2325 = (r2324×n2324 + r25×n25) / (n2324+n25)、単年±30%キャップ付き
"""
import sys, os, time
import numpy as np
import pandas as pd
...
CANDIDATES = [
    ...
    # クラス系（1走前はFORCED_BASEに移動）
    '2走前_クラス差', '3走前_クラス差',
    '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
    # 馬場
    '1走前_馬場状態', '道悪_平均着順_近5走',
    # 勝率統計
    '種牡馬_勝率', '騎手コース_r100_勝率', '調教師コース_r100_勝率',
    # 複勝率
    '近3走_複勝率',
    # 間隔・その他
    '間隔', '性別_num', '同会場_平均着順_近5走',
    # ※除外: '騎手変更'（穴馬バイアス）
]
...
def main():
    t0 = time.time()
    print("=" * 90)
    print("  ダ長距離 特徴量探索 v2 — 選択指標=2325(2023-25合算) OOS ROI")
    print("  1走前_クラス差強制ベース版。セグメント: ダ & 距離>1400m & クラス_rank≠1.0")
    print("=" * 90)
