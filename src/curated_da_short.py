# coding: utf-8
"""
curated_da_short.py - ダート短距離 意味のある特徴量限定版 Forward Greedy
* 候補を「解釈できる特徴量」に絞る（謎特徴を除外）
* 除外: 3走前/4走前クラス差, 1走前馬場状態, 産地/生産者/母父馬勝率
* 選択基準: Δ2324 > +0.5pp のみ
...
ALL_CANDIDATES = [
    ...
    '近5走_タイム指数平均', '近5走_タイム指数_max', '近5走_タイム指数_min',
    'タイム指数_近5走_slope', 'タイム指数_近3走_slope',
    '1走前_タイム指数', '前走着差タイム',

    # クラス（直前のみ - 遠い過去は除外）
    '1走前_クラス差', '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
    'クラス_rank',
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("  ダート短距離 意味特徴量限定版 Forward Greedy")
    print("  選択基準: Δ2324 > +0.5pp のみ")
    print("  候補: 解釈可能な特徴量のみ（謎特徴除外）")
