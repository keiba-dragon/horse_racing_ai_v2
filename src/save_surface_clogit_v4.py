...
"""
芝ダ別 条件付きロジット v4
- 学習: 2013-2021 (val だった 2021 を学習に追加)
- val: 2022 のみ (isotonic calibration + 早期停止)
- OOS: 2023+ (同じ)
...
def main():
    ...
    for surf in ['芝', 'ダ']:
        ...
        trn_s = df[(df['日付_num'] >= 130101) & (df['日付_num'] < 220101) & (df['surface'] == surf)].sort_values('race_id').reset_index(drop=True)
        val_s = df[(df['日付_num'] >= 220101) & (df['日付_num'] <= 221231) & (df['surface'] == surf)].sort_values('race_id').reset_index(drop=True)
        oos_s = df[(df['日付_num'] >= 230101) & (df['surface'] == surf)].sort_values('race_id').reset_index(drop=True)

        print(f'  学習: {trn_s["race_id"].nunique():,}R (2013-2021)')
        print(f'  val:  {val_s["race_id"].nunique():,}R (2022 のみ)')
        print(f'  OOS:  {oos_s["race_id"].nunique():,}R (2023+)')
        ...
        pred['rank_model'] = pred.groupby('race_id')['calib_prob'].rank(
    ...
    print(f'\n{"="*50}')
    print('芝+ダ合算 OOS 評価 (v4: 学習2013-2021, val2022)')
    print('='*50)
    ...
    print(mark)
    print('[比較] v1(学習2013-2020, val2021-2022): ROI=-0.132')

    out_pkl = os.path.join(MODEL_DIR, 'surface_clogit_v4.pkl')
