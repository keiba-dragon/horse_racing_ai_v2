# coding: utf-8
"""芝長距離（芝2001m以上）のベースライン確認 - 旧320特徴モデル（芝 artifact）"""
...
print(f"芝 artifact: {len(feats)}基底特徴量, beta:{beta.shape[0]}次元")
...
# 芝長距離フィルタ: 2001m以上
df_long = df[(df['surface'] == '芝') & (dm > 2000)].copy()
df_long['dist_m'] = dm[df_long.index]
df_long = add_computed_features(df_long)

print(f"\n芝長距離 全体: {len(df_long):,}行")
print(f"距離構成:")
for d, n in df_long['dist_m'].value_counts().sort_index().items():
    print(f"  {int(d)}m: {n:,}行")

oos_2324 = df_long[(df_long['日付_num'] >= 230101) & (df_long['日付_num'] < 250101)]
oos_2025 = df_long[(df_long['日付_num'] >= 250101) & (df_long['日付_num'] < 260101)]
oos_2026 = df_long[df_long['日付_num'] >= 260101]
print(f"\nOOS: 2324:{oos_2324['race_id'].nunique()}R  "
      f"2025:{oos_2025['race_id'].nunique()}R  "
      f"2026:{oos_2026['race_id'].nunique()}R")

missing = [c for c in feats if c not in df_long.columns]
print(f"\n欠けている特徴量: {len(missing)}個")
for c in missing:
    df_long[c] = np.nan

oos = df_long[df_long['日付_num'] >= 230101].copy()
...
print(f"\n{'='*55}")
print(f"  芝長距離(2001m以上) ベースライン (旧芝artifact)")
print(f"{'='*55}")
