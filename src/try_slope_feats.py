# coding: utf-8
"""
try_slope_feats.py - スロープ・トレンド系特徴の精密テスト
  タイム指数_近5走_slope が唯一 +0.47pp 確認済み
  残りのスロープ系を個別テスト → 有望ならステップワイズ追加
"""
import sys, os
import numpy as np
import pandas as pd
...
def main():
    ...
    print('=== スロープ・トレンド系特徴テスト (BASE_24 + L2=0.006) ===')
    print()
    ...
    print(f'★ ポジティブ: {[(c, f"{d:+.2%}") for d,c in positives]}')
    print()

    # ステップワイズ追加
    current_feats = BASE_24.copy()
    current_c = c0
    added = []
    print('ステップワイズ:')
    for d, c in positives:
        ...
        comb = show(f'BASE_24+{added+[c]}', roi, current_c)
        if comb > current_c:
    ...
    print()
    print(f'最終: BASE_24 + {added}')
    print(f'最終25+26: {current_c:+.2%}  (Δ={current_c-c0:+.2%} from BASE_24)')
