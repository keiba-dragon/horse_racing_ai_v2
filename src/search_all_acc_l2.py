# coding: utf-8
"""
search_all_acc_l2.py - 全5セグメント L2変更探索
usage: python search_all_acc_l2.py 0.003
       python search_all_acc_l2.py 0.010
現行 L2=0.006 と異なる正則化で別の局所最適を探索
"""
...
L2 = float(sys.argv[1]) if len(sys.argv) > 1 else 0.003
...
def adam_fit(X_tr, y_tr, gs_tr, n_tr, nr_tr, X_va, y_va, gs_va, n_va, nr_va, l2=None):
    if l2 is None: l2 = L2
...
def eval_feats(feats, dfs):
    ...
    try:
        ...
        beta = adam_fit(X_tr,y_tr,gs_tr,n_tr,nr_tr,X_va,y_va,gs_va,n_va,nr_va,l2=L2)
...
def save_seg(name, feats, seg, version_suffix):
    ...
    ver = f'{name}_acc_L2_{str(L2).replace(".","")}{version_suffix}'
    acc_pkg={
...
def main():
    print(f'全5セグメント L2={L2} 探索  MAX_FEATS={MAX_FEATS}')
    ...
    for name in ['芝長','芝中','芝短','ダ長','ダ短']:
        print(f'\n{"="*60}')
        print(f'  {name}  L2={L2}  目標={FAV[name]:.4f}')
        print(f'{"="*60}')
        seg=get_seg(name)
        df_trn=seg[(seg['日付_num']>=130101)&(seg['日付_num']<220101)]
        df_val=seg[(seg['日付_num']>=220101)&(seg['日付_num']<=221231)]
