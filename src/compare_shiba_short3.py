# coding: utf-8
"""
compare_shiba_short3.py - 芝短距離 Round3
* Round2 で 騎手/調教師コースWR は逆効果と判明 → 除外
* nv1に1〜2特徴ずつ追加して何が安全かを特定する
* 目標: タイ率↓ かつ 25+26 が nv1(+14.85%)を大きく割らない
"""
import sys, os, time
import numpy as np
import pandas as pd
...
ALL_FEATS = [
    ...
    '前走着差タイム', '1走前_タイム指数', '近5走_上り3F平均',
    '馬距離_勝率', '馬距離_複勝率', 'コース枠_r200_勝率',
    '1走前_クラス差', '2走前_クラス差', '1走前_クラス調整着順',
    '近5走_クラス調整_平均着順', '間隔', '馬体重', '斤量', '馬番',
...
SETS = {
    'A: nv1(参考)':                NV1,
    'B: nv1+間隔':                 NV1 + ['間隔'],
    'C: nv1+馬距離WR':             NV1 + ['馬距離_勝率'],
    'D: nv1+近5走上3F':            NV1 + ['近5走_上り3F平均'],
    'E: nv1+前走着差タイム':        NV1 + ['前走着差タイム'],
    'F: nv1+1走前クラス差':         NV1 + ['1走前_クラス差'],
    'G: nv1+コース枠WR':            NV1 + ['コース枠_r200_勝率'],
    'H: nv1+間隔+馬距離WR':        NV1 + ['間隔', '馬距離_勝率'],
    'I: nv1+間隔+近5走上3F':       NV1 + ['間隔', '近5走_上り3F平均'],
    'J: nv1+馬距離WR+近5走上3F':  NV1 + ['馬距離_勝率', '近5走_上り3F平均'],
    'K: nv1+間隔+馬距離+上3F':     NV1 + ['間隔', '馬距離_勝率', '近5走_上り3F平均'],
    'L: nv1+クラス差2+間隔':        NV1 + ['1走前_クラス差', '2走前_クラス差', '間隔'],
    'M: nv1+クラス調整+間隔':       NV1 + ['1走前_クラス調整着順', '間隔'],
    'N: nv1+タイム指数':            NV1 + ['1走前_タイム指数'],
}
...
def tie_rate_2026(scored_df):
    ...
    scored_df['_rm'] = scored_df.groupby('race_id')['prob'].rank(ascending=False, method='min')
    ties = scored_df[scored_df['_rm'] == 1].groupby('race_id').size()
    return (ties > 1).mean()
...
def main():
    ...
    NV1_ROI = +0.1485
    print("=" * 80)
    print("  芝短距離 Round3 — 1〜3特徴ずつ追加（騎手/調教師WR除外）")
