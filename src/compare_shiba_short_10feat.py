# coding: utf-8
"""
compare_shiba_short_10feat.py - 芝短距離 10特徴探索（2323指標）
  選択指標: 2323 OOS ROI  (25+26は報告のみ)
  nv1ベース(4特徴)から greedy + pair search で 10 特徴を目指す
  セグメント: 芝 ≤1400m
"""
...
BASE_NV1 = ['1走前_3角', '芝ダ転向', '距離変化_前走', '1走前_脚質_num']
...
def load_segment():
    ...
    for col in BASE_NV1 + CANDIDATES:
...
def main():
    t0 = time.time()
    print("=" * 90)
    print("  芝短距離 10特徴探索 — 選択指標=2323 OOS ROI")
    print(f"  ベースnv1(4特徴): {BASE_NV1}")
    print("  ※ 旧nv2は25+26選択。今回は2323基準でやり直し")
    print("=" * 90)
    ...
    r_base, c_base, r25_b, r26_b, valid_base, _ = eval_feats(
        df_trn, df_val, oos_2324, oos_2025, oos_2026, BASE_NV1)
    print(f"\n[ベースnv1(4特徴)] 2323={r_base*100:+.2f}%  25+26={c_base*100:+.2f}%")

    selected = list(BASE_NV1)
