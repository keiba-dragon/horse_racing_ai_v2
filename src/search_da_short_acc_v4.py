# coding: utf-8
"""
search_da_short_acc_v4.py - ダ短 ゼロシードから再探索
目標: 1番人気 34.90% を超える
現状: v3=30.79% (gap -4.11pp)
新候補: 1走前_単勝オッズ, PCI系, 前走上り, 過去走タイム指数等を追加
"""
...
CANDIDATES = [
    ...
    '芝ダ転向', '馬体重', '騎手コース_r100_勝率', '騎手会場_r100_勝率',
    '騎手コース距離_r100_勝率', '騎手距離_r100_勝率',
    '調教師コース_r100_勝率', '種牡馬_勝率',
    '近3走_複勝率', '近3走_勝率',
    '同会場_複勝率_近5走', '同会場_平均着順_近5走', '同距離帯_平均着順_近5走',
    '道悪_平均着順_近5走', '同馬場_平均着順_近5走',
    '1走前_3角', '1走前_4角', '1走前_脚質_num',
    '4角位置_近3走_slope', '馬_r20_勝率',
]
...
def save_seg(feats, seg):
    ...
    acc_pkg = {
        'segment': 'ダ短', 'scaler': scaler, 'coef': beta, 'feat_cols': valid, 'isotonic': iso,
        'acc_2325': acc_2325, 'acc_2526': acc_2526, 'oos_roi_2526': roi_2526,
        'version': 'da_short_acc_v4',
        'note': f'v4: {len(feats)}特徴 ゼロシード acc_2325={acc_2325:.4f} 1番人気34.90%',
    }
...
def main():
    ...
    seg = df[(df['surface'] == 'ダ') & (dm <= 1400) & (df['クラス_rank'] != 1.0)].copy()
    ...
    candidates = [c for c in CANDIDATES if c in seg.columns and c not in FORCED_BASE]
    current = FORCED_BASE[:]
    best_score = eval_feats(current, dfs)
    print(f'ゼロシード (馬番・斤量のみ): acc_2325={best_score:.4f}')
    best_feats = current[:]
    ...
    while len(current) < MAX_FEATS and remaining:
        ...
        print(f'{mark}+{best_add:38s} acc_2325={best_score:.4f} gap={gap:+.4f} ({len(current)}f, {time.time()-t0:.0f}s)')
        sys.stdout.flush()
    ...
    print(f'\n最終: ダ短 acc_2325={best_score:.4f}  目標まで{FAV_TARGET-best_score:+.4f}  ({len(best_feats)}特徴)')
    ...
    if os.path.exists(acc_pkl):
        ...
        if 'ダ短' in pkg:
            existing_acc = pkg['ダ短'].get('acc_2325', 0.0)
