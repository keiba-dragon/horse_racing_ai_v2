# coding: utf-8
"""
compare_shiba_long2.py - 芝長距離（芝2001m以上）Round 2
* Round1: 全セットがベースライン(-16.90%)以下 → 別方向の特徴を探索
* 小サンプル(337R)向けに2-3特徴の単純モデルも確認
"""
import sys, os, time
import numpy as np
import pandas as pd
...
ALL_FEATS = [
    ...
    '間隔', '近5走_クラス調整_平均着順',
    '休み明けフラグ', '連闘フラグ', '斤量', '馬体重',
    '同会場_平均着順_近5走', '良馬場_平均着順_近5走',
]

SETS = {
    'I: 馬距離+脚質(2特徴)':       ['馬距離_勝率', '1走前_脚質_num'],
    'J: 馬距離+クラス調整':         ['馬距離_勝率', '1走前_クラス調整着順'],
    'K: 馬距離+間隔':              ['馬距離_勝率', '間隔'],
    'L: 調教師+前走着差':           ['調教師コース_r100_勝率', '前走着差タイム'],
    'M: 騎手+馬距離+脚質':          ['騎手コース_r100_勝率', '馬距離_勝率', '1走前_脚質_num'],
    'N: 前走着差+間隔+馬距離':       ['前走着差タイム', '間隔', '馬距離_勝率'],
    'P: 馬距離+5走クラス補正':       ['馬距離_勝率', '近5走_クラス調整_平均着順'],
    'Q: 馬距離_複勝率+クラス調整':   ['馬距離_複勝率', '1走前_クラス調整着順'],
    'R: 同会場成績+馬距離':          ['同会場_平均着順_近5走', '馬距離_勝率'],
    'S: 斤量+脚質+馬距離':          ['斤量', '1走前_脚質_num', '馬距離_勝率'],
}
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("  芝長距離（芝2001m以上）Round 2  baseline=-16.90% (25+26)")
