# coding: utf-8
"""
compare_shiba_long5.py - 芝長距離（芝2001m以上）Round 5
* Round4 best: X5=-6.08% (前走着差タイム, 距離変化_前走, 1走前_クラス差, 馬距離_勝率)
* X5 のバリエーションで安定性確認
* filtered baseline: -20.53%
"""
import sys, os, time
import numpy as np
import pandas as pd
...
ALL_FEATS = [
    '前走着差タイム', '距離変化_前走', '1走前_クラス差', '2走前_クラス差',
    '馬距離_勝率', '馬距離_複勝率', '騎手コース_r100_勝率', '調教師コース_r100_勝率',
    '1走前_3角', '1走前_脚質_num', '1走前_クラス調整着順',
    '間隔', '近5走_クラス調整_平均着順',
]

# X5: 前走着差タイム, 距離変化_前走, 1走前_クラス差, 馬距離_勝率 → -6.08%
SETS = {
    'X5: R4best(再確認)':           ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '馬距離_勝率'],
    'Y1: X5-距離変化':               ['前走着差タイム', '1走前_クラス差', '馬距離_勝率'],
    'Y2: X5-クラス差':               ['前走着差タイム', '距離変化_前走', '馬距離_勝率'],
    'Y3: X5+3角':                   ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '馬距離_勝率', '1走前_3角'],
    'Y4: X5+調教師':                 ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '馬距離_勝率', '調教師コース_r100_勝率'],
    'Y5: X5+脚質':                  ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '馬距離_勝率', '1走前_脚質_num'],
    'Y6: X5+5走補正':               ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '馬距離_勝率', '近5走_クラス調整_平均着順'],
    'Y7: X5+2走クラス差':            ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '馬距離_勝率', '2走前_クラス差'],
    'Y8: X5+間隔':                  ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '馬距離_勝率', '間隔'],
    'Y9: 馬距離複勝率版':              ['前走着差タイム', '距離変化_前走', '1走前_クラス差',
                                     '馬距離_複勝率'],
}
...
def main():
    ...
    FILTERED_BL = -0.2053
    print("=" * 72)
    print("  芝長距離（芝2001m以上）Round 5")
    print(f"  Round4 best X5=-6.08%  filtered baseline={FILTERED_BL*100:.2f}%")
    ...
    print(f"  filtered baseline比: {(best_comb - FILTERED_BL)*100:+.2f}pp")
    print(f"  Round4 best比 (X5=-6.08%): {(best_comb+0.0608)*100:+.2f}pp")
