# coding: utf-8
"""
search_acc_l2_003.py - 任意セグメント L2=0.003 ランダムサーチ
現在のpklベスト特徴量をSEEDとして使用
usage: python src/search_acc_l2_003.py <セグメント名> [N_ITER] [SEED]
"""
...
def main():
    ...
    seed_feats_raw = [f for f in existing[SEG_NAME].get('feat_cols', []) if not f.endswith('_isnan')] if SEG_NAME in existing else []

    print(f'ランダムサーチ L2={L2}: {SEG_NAME}  N_ITER={N_ITER}  SEED={SEED_RNG}')
    ...
    if best_score > existing_acc + 0.0001:
        ...
        acc_pkg = {
            ...
            'version': f'{SEG_NAME}_acc_L2_003_s{SEED_RNG}',
            'note': f'L2=0.003 rand s{SEED_RNG}: {len(best_feats)}特徴 acc_2325={acc_2325:.4f}',
        }
