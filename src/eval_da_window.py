# coding: utf-8
"""
eval_da_window.py - ダ_中長距離 学習期間ウィンドウ比較
古いデータが最近のパターンに合わないか調査
"""
import sys, os, time
import numpy as np
import pandas as pd
...
BASE_16 = [f for f in BASE_17 if f != '1走前_上り3F']  # 上り3F除外
...
def fmt(oos_roi):
    ...
    if '2025' in oos_roi and '2026' in oos_roi:
        ...
        n = n5 + n6
        parts.append(f'25+26:{comb:+.2%}({n}R)')
    return '  '.join(parts)
...
def main():
    ...
    configs = [
        ('trn:2013-2021(標準)', 130101, 220101, BASE_17, '17F'),
        ('trn:2016-2021',       160101, 220101, BASE_17, '17F'),
        ('trn:2018-2021',       180101, 220101, BASE_17, '17F'),
        ('trn:2019-2021',       190101, 220101, BASE_17, '17F'),
        ('trn:2013-2021',       130101, 220101, BASE_16, '16F(-上り3F)'),
        ('trn:2016-2021',       160101, 220101, BASE_16, '16F(-上り3F)'),
        ('trn:2018-2021',       180101, 220101, BASE_16, '16F(-上り3F)'),
    ]

    print(f'{"設定":<35} {"特徴":>8} {"trn行":>8} {"valNLL":>9}  {"結果"}')
    print('='*100)

    for label, trn_lo, trn_hi, feats, feat_label in configs:
        df_trn = df[(df['日付_num'] >= trn_lo) & (df['日付_num'] < trn_hi)]
        t0 = time.time()
        nll, roi = evaluate(df_trn, df_val, oos_parts, feats)
        elapsed = time.time() - t0
        if nll is None:
            print(f'{label:<35} データ不足')
            continue
        desc = f'{label} {feat_label}'
        print(f'{desc:<44} {len(df_trn):>8,} {nll:>9.5f}  {fmt(roi)}  [{elapsed:.0f}s]')
