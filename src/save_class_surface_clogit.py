...
- 未勝利 × {芝/ダ}: 4モデル合計
- 非未勝利 × {芝/ダ}:
- 仮説: 未勝利は別の特徴重みが有効 (surface split でROI -15.3%→-13.2% を再現する戦略)
"""
...
def main():
    ...
    # クラス_rank=2 が 未勝利
    df['is_maiden'] = (df['クラス_rank'] == 2).astype(int)
    df['class_grp'] = df['is_maiden'].map({1: '未勝利', 0: '非未勝利'})
    ...
    for cls_name, cls_mask_fn in [
        ('未勝利', lambda d: d['class_grp'] == '未勝利'),
        ('非未勝利', lambda d: d['class_grp'] == '非未勝利'),
    ]:
        for surf in ['芝', 'ダ']:
            key = f'{cls_name}_{surf}'
            df_cs = df[cls_mask_fn(df) & (df['surface'] == surf)].copy()

            trn_s = df_cs[(df_cs['日付_num'] >= 130101) & (df_cs['日付_num'] < 210101)].sort_values('race_id').reset_index(drop=True)
            val_s = df_cs[(df_cs['日付_num'] >= 210101) & (df_cs['日付_num'] <= 221231)].sort_values('race_id').reset_index(drop=True)
            oos_s = df_cs[df_cs['日付_num'] >= 230101].sort_values('race_id').reset_index(drop=True)
    ...
    print('\n--- クラス別 ---')
    for cls_name in ['未勝利', '非未勝利']:
        sub = top1_all[top1_all['class_grp'] == cls_name]
        if len(sub) == 0:
            continue
        won = sub['着順_num'] == 1
        r   = (sub.loc[won, 'odds_num'] * 100).sum() / (len(sub) * 100) - 1
        print(f'  {cls_name}: {len(sub):5d}R  win={won.mean():.3f}  ROI={r:+.3f}')
    ...
    out_pkl = os.path.join(MODEL_DIR, 'class_surface_clogit.pkl')
