# coding: utf-8
"""
search_acc_no_weight.py - 馬体重・馬体重増減を除外した的中率モデル探索
usage: python src/search_acc_no_weight.py <セグメント名> [N_ITER] [SEED]
"""
...
def main():
    ...
    # シードは既存特徴から体重系を除いたもの
    seed_feats_raw = [f for f in existing[SEG_NAME].get('feat_cols', [])
                      if not f.endswith('_isnan') and f not in {'馬体重', '馬体重増減'}
                      ] if SEG_NAME in existing else []

    print(f'ランダムサーチ（体重除外） L2={L2}: {SEG_NAME}  N_ITER={N_ITER}  SEED={SEED_RNG}')
    print(f'既存pkl: {existing_acc:.4f} ({len(seed_feats_raw)}特徴)  目標: {FAV:.4f}')
    ...
    seed_feats = [f for f in seed_feats_raw if f in seg.columns]

    rng = random.Random(SEED_RNG)
    best_feats = seed_feats[:]
    best_score = eval_feats(best_feats, dfs)
    print(f'SEED score: {best_score:.4f} ({len(best_feats)}特徴)  候補数: {len(cands)}')
    ...
    for i in range(N_ITER):
        ...
        if score > best_score:
            ...
            best_feats = current[:]
            gap = FAV - best_score
            mark = '★' if best_score >= FAV else ' '
            print(f'{mark}[{i+1:4d}/{N_ITER}] op={op:6s} {best_score:.4f} gap={gap:+.4f} '
                  f'({len(best_feats)}f, {time.time()-t0:.0f}s)', flush=True)
            if best_score >= FAV:
                print(f'*** {SEG_NAME}: 1番人気超え! ***')
                break

    print(f'\n最終 L2={L2}: {SEG_NAME} acc_2325={best_score:.4f}  目標まで{FAV-best_score:+.4f}')

    # 既存より改善した場合のみ保存（体重除外後スコアと比較）
    if best_score > existing_acc - 0.005:  # 既存から-0.5%以内なら採用
        print(f'採用 ({existing_acc:.4f}->{best_score:.4f}), 保存...')
        ...
        acc_pkg = {
            ...
            'version': f'{SEG_NAME}_acc_no_weight_s{SEED_RNG}',
            'note': f'体重除外 L2=0.003 s{SEED_RNG}: {len(best_feats)}特徴 acc_2325={acc_2325:.4f}',
        }
        existing2 = pickle.load(open(acc_pkl, 'rb'))
        existing2[SEG_NAME] = acc_pkg
        with open(acc_pkl, 'wb') as f: pickle.dump(existing2, f)
        print(f'  保存完了: {SEG_NAME}')
    else:
        print(f'既存より大幅に悪化 ({existing_acc:.4f} vs {best_score:.4f}), スキップ')
