# coding: utf-8
"""
compare_shiba_mid4.py - 芝中距離（芝1401m-2000m）Round 4
* Round3 best: AB=+6.99% (調教師コース+馬距離+クラス調整着順+5走クラス補正)
* AB のバリエーションで安定性確認・さらなる改善探索
"""
import sys, os, time
import numpy as np
import pandas as pd
...
ALL_FEATS = [
    ...
    '間隔', '近5走_クラス調整_平均着順', '近5走_クラス補正スコア',
    '最大クラス差_近5走', '近5走_クラス補正スコア',
]

# Round3: AB=+6.99% (best), S=-2.88%, U=+1.59%, W=+1.54%
# AB: 調教師コース_r100_勝率, 馬距離_勝率, 1走前_クラス調整着順, 近5走_クラス調整_平均着順
SETS = {
    'AB: R3best(再確認)':          ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順'],
    'AC: AB+脚質':                 ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
                                    '1走前_脚質_num'],
    'AD: AB+3角':                  ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
                                    '1走前_3角'],
    'AE: AB-クラス調整着順':        ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '近5走_クラス調整_平均着順'],
    'AF: AB+騎手':                 ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
                                    '騎手コース_r100_勝率'],
    'AG: 調教師+馬距離+5走補正':    ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '近5走_クラス補正スコア'],
    'AH: AB+クラス差':              ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
                                    '1走前_クラス差'],
    'AI: AB+間隔':                 ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
                                    '間隔'],
    'AJ: 5走のみ代替(複勝率)':      ['調教師コース_r100_勝率', '馬距離_複勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順'],
}
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("  芝中距離（芝1401m-2000m）Round 4")
    print("  Round3 best: AB=+6.99%  baseline=-22.21%")
    ...
    print(f"  Round3 best比 (AB=+6.99%): {(best_comb-0.0699)*100:+.2f}pp")
