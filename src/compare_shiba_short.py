# coding: utf-8
"""
compare_shiba_short.py - 芝短距離（芝≤1400m）特徴量セット直接比較
* 2324/2025/2026/25+26 を全セット同時評価（2324を選択基準にしない）
* ベースライン: -26.54% (旧芝artifact)
"""
import sys, os, time
import numpy as np
import pandas as pd
...
SETS = {
    'A: nv1(全体Greedy4特徴)':   ['1走前_3角', '芝ダ転向', '距離変化_前走', '1走前_脚質_num'],
    'B: 前走内容3特徴':           ['1走前_クラス調整着順', '前走着差タイム', '1走前_3角'],
    'C: コース脚質適性':          ['コース枠_r200_勝率', 'コース脚質_r200_勝率', '馬距離_勝率', '1走前_脚質_num'],
    'D: A+前走スコア':            ['1走前_3角', '芝ダ転向', '距離変化_前走', '1走前_脚質_num',
                                   '1走前_クラス調整着順', '前走着差タイム'],
    'E: A+コース騎手':            ['1走前_3角', '芝ダ転向', '距離変化_前走', '1走前_脚質_num',
                                   'コース枠_r200_勝率', '騎手コース_r100_勝率'],
    'F: 1200mGreedy5特徴':        ['1走前_タイム指数', '馬体重', 'コース枠_r200_複勝率', '性別_num', '近5走_上り3F平均'],
    'G: B+コース':                ['1走前_クラス調整着順', '前走着差タイム', '1走前_3角',
                                   'コース枠_r200_勝率', 'コース脚質_r200_勝率'],
    'H: クラス差+脚質':           ['1走前_クラス差', '2走前_クラス差', '1走前_脚質_num', '1走前_3角'],
}
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("  芝短距離（芝≤1400m）特徴量セット直接比較")
    print("  ベースライン: -26.54% (旧芝artifact, 2324OOS)")
    ...
    print(f"  ベースライン比: {(best_comb+0.2654)*100:+.2f}pp")
