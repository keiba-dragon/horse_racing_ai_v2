# coding: utf-8
"""
compare_shiba_mid.py - 芝中距離（芝1401m-2000m）特徴量セット直接比較
* 2324/2025/2026/25+26 を全セット同時評価（2324を選択基準にしない）
* ベースライン: 旧芝artifact (baseline_shiba_mid.py で確認)
"""
import sys, os, time
import numpy as np
import pandas as pd
...
SETS = {
    'A: 芝短nv1(参照)':          ['1走前_3角', '芝ダ転向', '距離変化_前走', '1走前_脚質_num'],
    'B: 上り3F+脚質+前走':       ['近5走_上り3F平均', '1走前_脚質_num', '1走前_3角', '距離変化_前走'],
    'C: 前走成績+上り':          ['1走前_クラス調整着順', '前走着差タイム', '近5走_上り3F平均'],
    'D: A+上り3F':               ['1走前_3角', '芝ダ転向', '距離変化_前走', '1走前_脚質_num',
                                   '近5走_上り3F平均'],
    'E: コース適性+前走':        ['馬距離_勝率', 'コース枠_r200_勝率', '1走前_クラス調整着順',
                                   '前走着差タイム'],
    'F: タイム指数中心':         ['1走前_タイム指数', '距離変化_前走', '1走前_クラス差',
                                   '近5走_上り3F平均'],
    'G: クラス差+脚質+コース':   ['1走前_クラス差', '2走前_クラス差', '1走前_脚質_num',
                                   '馬距離_勝率'],
    'H: 騎手+クラス+脚質':       ['騎手コース_r100_勝率', '1走前_クラス調整着順',
                                   '1走前_脚質_num', '1走前_3角'],
}
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("  芝中距離（芝1401m-2000m）特徴量セット直接比較")
