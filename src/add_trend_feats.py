# coding: utf-8
"""
add_trend_feats.py - 上がり調子系特徴を 21F ベースに個別追加してテスト
"""
import sys, os
import numpy as np
import pandas as pd
...
TREND_CANDIDATES = [
    ...
    '着順_加速度',               # 負=急改善
    # 複合トレンド
    '近走_改善トレンド',          # 3走前タイム指数 - 1走前タイム指数（後方互換）
    # 上り3F・4角
    '上り3F_近3走_slope',
    '4角位置_近3走_slope',
    # 馬体重トレンド
...
def main():
    ...
    # NaN率確認
    print('上がり調子系 NaN率（ダ中長距離・訓練データ）:')
    for c in TREND_CANDIDATES:
        if c in df.columns:
            nan_tr = df_trn[c].isna().mean()
            print(f'  {c:<35} NaN訓練:{nan_tr:.1%}')
        else:
            print(f'  {c:<35} [列なし]')
    print()
