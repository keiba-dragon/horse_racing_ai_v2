# coding: utf-8
"""
fine_tune_l2.py - L2=0.01周辺を細かくグリッドサーチ + 2023-24も表示
"""
import sys, os
import numpy as np
import pandas as pd
...
def evaluate(df_trn, df_val, oos_parts, feats, l2=0.0):
    ...
    beta, _ = adam_fit_l2(X_tr, y_tr, gs_tr, n_tr, nr_tr,
...
def main():
    ...
    print(f'{"L2":>8}  {"2324":>8}  {"2025":>8}  {"2026":>8}  {"25+26":>8}')
    print('='*52)
    best_comb = -99; best_l2 = 0
    for l2 in [0.0, 0.002, 0.004, 0.006, 0.008, 0.010, 0.012, 0.015, 0.020, 0.025, 0.030]:
        _, roi = evaluate(df_trn, df_val, oos_parts, BASE_24, l2=l2)
        r24, n24, _ = roi.get('2324', (0, 1, 0))
        r25, n25, _ = roi.get('2025', (0, 1, 0))
        r26, n26, _ = roi.get('2026', (0, 1, 0))
        comb = (r25*n25 + r26*n26) / (n25+n26)
        mark = ' ★' if comb > best_comb else ''
        print(f'{l2:>8.4f}  {r24:>+8.2%}  {r25:>+8.2%}  {r26:>+8.2%}  {comb:>+8.2%}{mark}')
        if comb > best_comb:
            best_comb = comb; best_l2 = l2

    print(f'\n最良 L2={best_l2}: 25+26={best_comb:+.2%}')
    print(f'21F ベースライン: -19.70%')
    print(f'累計改善: {best_comb - (-0.197):+.2%}')
