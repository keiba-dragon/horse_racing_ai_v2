# coding: utf-8
"""
compare_shiba_long.py - 芝長距離（芝2001m以上）特徴量セット直接比較
* 障害レース除外: クラス_rank が NaN の行を除外
* 2324/2025/2026/25+26 を全セット同時評価（2324を選択基準にしない）
* ベースライン: 旧芝artifact = -16.90% (25+26)
"""
import sys, os, time
import numpy as np
import pandas as pd
...
# 芝中 AI (best): 調教師コース+馬距離+クラス調整着順+近5走クラス補正+間隔 → +8.81%
# 芝長は距離が長い → スタミナ・ペース系・コース適性が重要
SETS = {
    'A: 芝中AI参照(4特徴)':        ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順'],
    'B: 芝中AI(5特徴=間隔追加)':   ['調教師コース_r100_勝率', '馬距離_勝率',
                                    '1走前_クラス調整着順', '近5走_クラス調整_平均着順',
                                    '間隔'],
    'C: 脚質+前走着差':            ['1走前_3角', '1走前_脚質_num', '距離変化_前走', '芝ダ転向'],
    'D: 上り3F+クラス':            ['近5走_上り3F平均', '1走前_クラス調整着順', '前走着差タイム'],
    'E: コース+騎手+クラス':        ['馬距離_勝率', '騎手コース_r100_勝率', '1走前_クラス調整着順'],
    'F: クラス差+脚質+コース':      ['1走前_クラス差', '2走前_クラス差',
                                    '1走前_脚質_num', '馬距離_勝率'],
    'G: 調教師+馬距離+脚質':        ['調教師コース_r100_勝率', '馬距離_勝率', '1走前_脚質_num',
                                    '近5走_クラス調整_平均着順'],
    'H: 調教師+馬距離のみ':         ['調教師コース_r100_勝率', '馬距離_勝率'],
}
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("  芝長距離（芝2001m以上）特徴量セット直接比較")
    print("  ※ 障害除外(クラス_rank notna)  baseline=-16.90% (25+26)")
    ...
    print(f"\n距離構成 (全データ, 障害除外後):")
    for d, n in df['dist_m'].value_counts().sort_index().items():
        print(f"  {int(d)}m: {n:,}行")
    ...
    print(f"\n{'='*72}")
    print(f"  ベスト: {best_name}  25+26={best_comb*100:.2f}%")
    print(f"  baseline比: {(best_comb+0.1690)*100:+.2f}pp")
