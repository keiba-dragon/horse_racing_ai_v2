# coding: utf-8
"""
search_fukusho_2325.py - 複勝版 Greedy forward selection
  損失: neg_log_lik_fukusho (P(top3) 最大化)
  選択指標: 複勝ROI 2325合算 (23-24 + 2025, 単年±30%キャップ)
  特徴数: 最大10
  保存先: accuracy_fukusho_model.pkl

usage:
  python src/search_fukusho_2325.py ダ長
  python src/search_fukusho_2325.py ダ短
  python src/search_fukusho_2325.py 芝短
  python src/search_fukusho_2325.py 芝中
  python src/search_fukusho_2325.py 芝長
"""
...
CANDIDATES = [
    ...
    '1走前_馬場状態', '良馬場_平均着順_近5走',
    '種牡馬_勝率',
    '同会場_平均着順_近5走', 'コース脚質_r200_勝率',
    '騎手コース_r100_勝率', '調教師コース_r100_勝率',
    '間隔', '性別_num', '近3走_複勝率',
    'コース枠_r200_複勝率', '種牡馬_芝_複勝率', '種牡馬_ダ_複勝率',
    '近5走_クラス調整_平均着順', '近10走_勝率',
]
...
def get_y_top3(df_sorted):
    return (df_sorted['着順_num'] <= 3).astype(float).values


def adam_fit_fukusho(X_tr, y_tr, gs_tr, n_tr, nr_tr,
    ...
    for epoch in range(1, N_EPOCHS + 1):
        _, grad = neg_log_lik_fukusho_and_grad(beta, X_tr, y_tr, gs_tr, n_tr, nr_tr)
        t += 1
        m = b1 * m + (1 - b1) * grad
        v = b2 * v + (1 - b2) * grad ** 2
        beta -= LR * (m / (1 - b1 ** t)) / (np.sqrt(v / (1 - b2 ** t)) + eps)
        if epoch % 10 == 0:
            vl, _ = neg_log_lik_fukusho_and_grad(beta, X_va, y_va, gs_va, n_va, nr_va)
            if vl < best_val:
...
def fukusho_roi(oos_sorted, beta, valid, scaler):
    """モデルrank-1馬の複勝ROIを計算"""
    ...
    nr = len(top1)  # 全ベット数（= 全レース数）
    if nr == 0:
        return float('nan'), 0
    top1['_fuku_pay'] = pd.to_numeric(top1['複勝配当'], errors='coerce')
    # 3着以内かつ配当データあり = 的中
    top1_hit = top1[top1['_fuku_pay'].notna() & (top1['着順_num'] <= 3)].copy()
    payout = (top1_hit['_fuku_pay'] / 100).sum()
    roi = (payout - nr) / nr
    return roi, nr
...
def eval_feats(df_trn, df_val, oos_2324, oos_2025, oos_2026, feats):
    ...
    try:
        X_tr, _, gs_tr, n_tr, nr_tr, scaler, *_ = prepare(
            df_trn, valid, top_idx=None, top_idx3=None, fit=True)
        y_tr = get_y_top3(df_trn.sort_values('race_id').reset_index(drop=True))
        X_va, _, gs_va, n_va, nr_va, *_ = prepare(
            df_val, valid, scaler=scaler, top_idx=None, top_idx3=None)
        y_va = get_y_top3(df_val.sort_values('race_id').reset_index(drop=True))
        beta = adam_fit_fukusho(X_tr, y_tr, gs_tr, n_tr, nr_tr,
                                X_va, y_va, gs_va, n_va, nr_va)
    except Exception as e:
        return float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), valid, None, None

    r2324, n2324 = fukusho_roi(oos_2324.sort_values('race_id').reset_index(drop=True), beta, valid, scaler)
    r25,   n25   = fukusho_roi(oos_2025.sort_values('race_id').reset_index(drop=True), beta, valid, scaler)
    r26,   n26   = fukusho_roi(oos_2026.sort_values('race_id').reset_index(drop=True), beta, valid, scaler)
...
def main():
    t0 = time.time()
    print("=" * 90)
    print(f"  {SEG_NAME} 複勝版 Greedy forward selection")
    print(f"  選択指標: 複勝ROI 2325合算 (単年±{ROI_CAP:.0%}キャップ)  最大{MAX_FEATS}特徴")
    print("=" * 90)
    ...
    print("\n=== Greedy forward selection (複勝ROI 2325) ===")
    ...
    print(f"  最良 2325複勝ROI: {best_r2325*100:+.2f}%")
    print(f"{'='*90}")

    # ── 保存 ────────────────────────────────────────────────────────────────
    if best_beta is None or best_scaler is None:
        print("保存スキップ（betaなし）")
        return
    ...
    valid_final = [c for c in expanded_final
                   ...
                   and df_trn[c].isna().mean() < 1.0
                   and df_trn[c].std(ddof=0) > 0]
    X_tr_f, _, gs_tr_f, n_tr_f, nr_tr_f, sc_f, *_ = prepare(
        df_trn, valid_final, top_idx=None, top_idx3=None, fit=True)
    y_tr_f = get_y_top3(df_trn.sort_values('race_id').reset_index(drop=True))
    X_va_f, _, gs_va_f, n_va_f, nr_va_f, *_ = prepare(
        df_val, valid_final, scaler=sc_f, top_idx=None, top_idx3=None)
    y_va_f = get_y_top3(df_val.sort_values('race_id').reset_index(drop=True))
    beta_f = adam_fit_fukusho(X_tr_f, y_tr_f, gs_tr_f, n_tr_f, nr_tr_f,
                               X_va_f, y_va_f, gs_va_f, n_va_f, nr_va_f)
    raw_val = segment_softmax(X_va_f @ beta_f, gs_va_f, n_va_f)
    iso = IsotonicRegression(out_of_bounds='clip')
    iso.fit(raw_val, y_va_f)

    model_path = os.path.join(BASE_DIR, 'models', 'accuracy_fukusho_model.pkl')
    try:
