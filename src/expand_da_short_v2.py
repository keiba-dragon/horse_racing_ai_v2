# coding: utf-8
"""
expand_da_short_v2.py - ダート短距離 4特徴起点 ガード条件さらに緩和
START: [近5走_上り3F平均, 3走前_クラス差, キャリア_浅い, 2走前_クラス差]
採用基準: Δ25+26 > 0 かつ 2324 > 0 (正のROI維持)
"""
import sys, os, time
import numpy as np
import pandas as pd
...
def main():
    t0 = time.time()
    print("=== ダート短距離 Forward Greedy v2 (ガード: 2324>0 かつ Δ25+26>0) ===\n")
    df = load_segment()
    ...
    while remaining:
        ...
        for cand in remaining:
            ...
            # 採用基準: Δ25+26 > 0 かつ 2324 > 0 (正ROI維持)
            good = delta_comb > 0 and r2324 > 0 and actually_added
            sym = '✓' if good else '✗'
            note = '' if actually_added else ' [NaN>65%]'
            print(f"  {sym} +{cand}{note}  2324:{r2324*100:.2f}%(Δ{(r2324-best_2324)*100:+.2f}%)  "
