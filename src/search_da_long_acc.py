...
L2 = 0.006
MAX_FEATS = 40
ACC_TARGET = 0.30  # まず30%を目指す

FORCED_BASE = ['馬番', '斤量']

# v6ベスト35特徴をシードとして途中から再開
SEED_FEATS = [
    '馬番', '斤量', '近3走_複勝率', '騎手コース_r100_勝率', '1走前_クラス調整着順',
    '近5走_タイム指数_max', '馬距離_勝率', '種牡馬_勝率', 'タイム指数_加速度',
    '近5走_タイム指数平均', '近5走_上り3F平均', '近5走_上り3F_std', '1走前_クラス差',
    'ブリンカー変更', '1走前_3角', '間隔', '距離変化_前走', '前走着差タイム',
    '騎手変更', '輸送有無', 'コース枠_r200_勝率', '馬体重増減', 'コース脚質_r200_勝率',
    '1走前_馬場状態', '1走前_脚質_num', '近10走_勝率', '2走前_クラス差',
    '道悪_平均着順_近5走', 'コース馬場_r200_勝率', '種牡馬_ダ_勝率',
    '近3走_勝率', 'コース枠_r200_複勝率', '馬_r20_勝率', '性別_num', '騎手会場_r100_勝率',
]
...
def load_segment():
    ...
    df = df[(df['surface'] == 'ダ') & (dm > 1400) & (df['クラス_rank'] != 1.0)].copy()
    df['dist_m'] = dm[df.index]
    df = add_computed_features(df)

    # 輸送有無: 今走開催地と前走開催地が異なる = 1
    ...
    if '前走_距離_m' in df.columns:
        prev_dist = pd.to_numeric(df['前走_距離_m'], errors='coerce')
        df['前走_長距離フラグ'] = (prev_dist >= 1800).astype(float)
        df.loc[prev_dist.isna(), '前走_長距離フラグ'] = float('nan')
...
def main():
    t0 = time.time()
    print("=" * 90)
    print("  ダ長距離 特徴量探索 — 選択指標=的中率(acc_2325)  ※ROIモデルとは別物")
    print("  セグメント: ダ & 距離>1400m & クラス_rank≠1.0")
    print("=" * 90)
    ...
    print(f"  参考ROI: 25+26={roi_s['25+26']*100:+.1f}%")
    print(f"  目標: acc_2325 >= {ACC_TARGET:.2f}  (1番人気: 34.1%)")
    best_acc = acc_seed if not np.isnan(acc_seed) else float('-inf')

    print("\n=== Greedy forward selection (的中率acc_2325で選択、目標34%) ===")
    while len(selected) < MAX_FEATS and remaining and best_acc < ACC_TARGET:
        round_res = []
