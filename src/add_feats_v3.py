# coding: utf-8
"""
add_feats_v3.py - 未試験の parquet カラムを全網羅して 21F ベースに追加テスト
"""
import sys, os
import numpy as np
import pandas as pd
...
def main():
    ...
    for cand in CANDIDATES:
        ...
        print(f'{sym} {cand:<35} Δ25+26:{d_comb:>+7.2%}  Δ25:{d25:>+7.2%}  Δ26:{d26:>+7.2%}  NaN:{nan_trn:.0%}  {agree}')

    results.sort(key=lambda x: -x[3])
    print()
    print(f'{"特徴量":<35} {"Δ25+26":>8} {"Δ2025":>8} {"Δ2026":>8}  合意?')
    print('='*80)
    for cand, d25, d26, d_comb, comb, both, nan_trn in results:
        sym = '✓' if d_comb > 0.003 else ('✗' if d_comb < -0.003 else '~')
        agree = '[両✓]' if both else ''
