# coding: utf-8
"""
fresh_da_short.py - ダート短距離 純粋Forward Greedy (空集合スタート)
BASE_25バイアスを排除し、ダ短専用特徴量セットをゼロから構築
採用基準: Δ2324 > +0.5pp かつ Δ25+26 >= -0.5pp
"""
import sys, os, time
import numpy as np
import pandas as pd
...
# 全候補 (BASE_25 + ADD_CANDIDATES 合体・重複除去)
ALL_CANDIDATES = [
...
def main():
    t0 = time.time()
    print("=== ダート短距離 Fresh Forward Greedy (空集合スタート) ===\n")
    df = load_segment()
    ...
    print("="*65)
    print("  Round 1: 最初の特徴量を選択 (2324 ROI最大)")
    print("="*65)
    ...
    for cand in avail:
        res = evaluate(df_trn, df_val, oos_2324, oos_2025, oos_2026, [cand])
        if res[0] is None:
            continue
        _, valid_t, r2324, r25, r26, ns = res
        if cand not in valid_t:
            continue
        n25, n26 = ns[1], ns[2]
        rcomb = comb2526(r25, n25, r26, n26)
        print(f"  [{cand}] 2324:{r2324*100:.2f}%  25:{r25*100:.2f}%  26:{r26*100:.2f}%  25+26:{rcomb*100:.2f}%")
        if r2324 > best_r2324:
            best_r2324 = r2324
            best_rcomb = rcomb
            best_feat = cand
    ...
    print(f"\n  → 初期採用: {best_feat}  2324:{best_r2324*100:.2f}%  25+26:{best_rcomb*100:.2f}%")
    current = [best_feat]
    best_2324 = best_r2324
    best_comb = best_rcomb
    remaining = [c for c in avail if c not in current]

    print(f"\n{'='*65}")
    print("  Forward Greedy (採用基準: Δ2324>+0.5pp かつ Δ25+26≥-0.5pp)")
    print(f"{'='*65}")
    ...
    while remaining:
        ...
        for cand in remaining:
            ...
            if good and delta_2324 > best_delta:
                best_delta = delta_2324
                best_cand_this = cand
