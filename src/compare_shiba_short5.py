# coding: utf-8
"""
compare_shiba_short5.py - 芝短距離 Round5
* Round4 best: P7=I+コース脚質WR (7特徴, 25+26=+34.10%, タイ=0.8%)
* P7 をベースに 3 特徴追加で 10 特徴を目指す
* 馬体重は有害確認済み → 除外
"""
import sys, os, time
import numpy as np
...
ALL_FEATS = [
    ...
    '近5走_上り3F平均', '間隔', 'コース脚質_r200_勝率',
    '同会場_平均着順_近5走', '良馬場_平均着順_近5走',
    '2走前_クラス差', '1走前_クラス差', '1走前_クラス調整着順',
    '近5走_クラス調整_平均着順',
    'コース枠_r200_勝率', '馬距離_勝率', '馬距離_複勝率',
    '前走着差タイム', '1走前_タイム指数',
    '斤量', '馬番', '性別_num',
]

# Round4 best P7
BASE_P7 = ['1走前_3角', '芝ダ転向', '距離変化_前走', '1走前_脚質_num',
            '間隔', '近5走_上り3F平均', 'コース脚質_r200_勝率']

SETS = {
    'P7: R4best(7特徴,参考)':           BASE_P7,
    'Q1: P7+同会場着順':                 BASE_P7 + ['同会場_平均着順_近5走'],
    'Q2: P7+クラス調整着順':              BASE_P7 + ['1走前_クラス調整着順'],
    'Q3: P7+近5走クラス調整':             BASE_P7 + ['近5走_クラス調整_平均着順'],
    'Q4: P7+斤量':                       BASE_P7 + ['斤量'],
    'Q5: P7+馬番':                       BASE_P7 + ['馬番'],
    'Q6: P7+コース枠WR':                 BASE_P7 + ['コース枠_r200_勝率'],
    'Q7: P7+1走前クラス差':               BASE_P7 + ['1走前_クラス差'],
    'Q8: P7+同会場+クラス調整':           BASE_P7 + ['同会場_平均着順_近5走',
                                                     '1走前_クラス調整着順'],
    'Q9: P7+同会場+クラス調整+斤量':      BASE_P7 + ['同会場_平均着順_近5走',
                                                     '1走前_クラス調整着順', '斤量'],
    'Q10: P7+近5走クラス調整+同会場+斤量': BASE_P7 + ['近5走_クラス調整_平均着順',
                                                      '同会場_平均着順_近5走', '斤量'],
    'Q11: P7+同会場+コース枠+斤量':       BASE_P7 + ['同会場_平均着順_近5走',
                                                     'コース枠_r200_勝率', '斤量'],
    'Q12: P7+クラス調整+近5走クラス+斤量': BASE_P7 + ['1走前_クラス調整着順',
                                                      '近5走_クラス調整_平均着順', '斤量'],
    'Q13: P7+馬距離WR+同会場+斤量':       BASE_P7 + ['馬距離_勝率',
                                                     '同会場_平均着順_近5走', '斤量'],
}
...
def main():
    ...
    P7_ROI = +0.3410
    print("=" * 82)
    print("  芝短距離 Round5 — P7(7特徴,+34.10%)に3特徴追加で10特徴へ")
    print(f"  ベースP7: {BASE_P7}")
    ...
    print(f"\n{'='*82}")
    print(f"  {'セット':38s}  {'2324':>7}  {'2025':>7}  {'2026':>7}  {'25+26':>7}  "
          f"{'タイ率':>6}  特徴数")
    print(f"  {'-'*78}")
    ...
    for name, feats in SETS.items():
        ...
        print(f"  {name:38s}  {r2324*100:+6.2f}%  {r25*100:+6.2f}%  {r26*100:+6.2f}%  "
    ...
    print(f"\n{'='*82}")
    print(f"  Round5 ベスト: {best_name}  25+26={best_comb*100:.2f}%")
    print(f"  P7比: {(best_comb - P7_ROI)*100:+.2f}pp")
